...
| **X-post comment 候補 画像添付 brand_opinion + brand_quote (IN_FLIGHT 2026-05-27)** | `doc/active/438-XPOST-brand-opinion-and-quote-images.md` / GH PENDING | x-post-mail-lane の comment 系候補 (GEMMA_BRANDING / build_x_post_from_article_info) に **毎時 fire で image を毎回添付**、 新規 Cloud Scheduler / Cloud Run job 一切作らない (¥10/月/job 削減 lock 維持)。 2 pattern: A=brand_opinion (反応元 og:image + ヨシラバー voice text overlay)、 B=brand_quote (引用元 og:image + 「発言者名「literal 60-180 字長 quote」」 overlay)。 共通: source 画像 native aspect / 強制 crop なし、 画像内 brand mark / 引用元 焼かない、 出典は alt text に「引用元: 媒体名」。 post text の hard rule (URL / ハッシュタグ / 媒体名 含めない) 維持。 Phase 1=A 単体、 Phase 2=B + long quote extractor + 発言者特定。 prerequisite=5/27 commit `776252d` (監督・コーチ member gate 拡張)。 deploy hygiene = .gcloudignore + feat/377 起点 + image tag に short sha。 post-deploy 24h `gcloud billing` 実測 verify 必須。 |
