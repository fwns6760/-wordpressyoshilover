...
|---|---|---|
| **X-POST brand voice quality framework (2026-05-20、 A-E 5 軸、 DRAFT GO 受領)** | `doc/active/414-X-POST-BRAND-VOICE-QUALITY-FRAMEWORK.md` | 元 9 axis hallucination 防止を user 追加 spec で A-E 5 軸に拡張。 **A 型分離 5 種** (速報事実/感情代弁/データ歴代/次の展開采配/ポジティブ若手): 5 prompt template + 自動選択。 **B voice persona** (411 既実装、 フーガ/缶詰)。 **C 精度 9 axis**: 順位 forbidden regex / 数値 whitelist (verified set 外 drop) / 率系 regex / prompt bad example / temperature 0.4 / published_date 7日超 drop / drop log 構造化 / 1軍 active filter (active_roster_filter.py 新規、 直近 14日 3試合) / persona=kandume 当日 only。 **D 炎上 6 check**: 強批判 / 断定 / 雑批判 / 監督批判 / 誤字 / 煽り 各 regex。 **E 試合前 7 テーマ**: 先発 / 昨日の流れ / 注目選手 / 打順変更 / 昇格 / 相性 / ファン反応 (pregame_themes.py 新規)。 7 commit 段階実装、 並走 agent (403) `ranking_article_publisher.py` 衝突 risk = commit 直列。 cost ¥0/post 維持、 spec 382 hard rule 不変。 411 (`3dd89e5`) base。 memory 起点軸 F-Z (timing / thread / A/B / mascot 等) は 414 scope 外。 GH Issue #90。 |
| **X-POST branding voice persona (2026-05-20 chat 起票、 LIVE_DEPLOYED 観察中)** | `doc/active/411-X-POST-BRANDING-VOICE-PERSONA.md` | user 仕様 lock: Tavily で候補 URL 拾う / 公式・NPB・球団・主要スポーツ紙優先 / Tavily answer 不使用、 URL 本文・媒体名・日付見る / 野球観戦の記事 / フーガ風 (長文分析) + 缶詰風 (試合中実況) 2 voice persona / 観戦ツイートは試合日 18時以降のみ。 392 (CLOSED) Gemma 4 + Tavily 基盤を拡張、 `_TAVILY_INCLUDE_DOMAINS` 2→9、 `_format_tavily_context` に published_date 注入、 prompt 2 persona split、 `is_giants_game_day` (insight.db games table) + `select_branding_persona` (18-21時 + 試合日 = 缶詰、 他 = フーガ)。 cost ¥0/post 維持。 並走: 394 active P0 hallucination fix 同 file commit 直列。 |
