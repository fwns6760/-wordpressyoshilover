...
| **YouTube caption short quote summary** | `385-INGEST-youtube-caption-short-quote-summary.md` | REVIEW_NEEDED。user GO。YouTube 字幕を長い 600 字 1 block ではなく、LLM なし / 推測なしで短い引用 + 要点表示に寄せる。実装: caption text を deterministic に文分割し、最大2 quote + 最大3要点 bullet に整形。推測補完なし、HTML escape、出典 / embed 維持。tests green、deploy待ち。env / Secret / Scheduler / RUN_DRAFT_ONLY / WP既存記事 / X / frontend は不可触。GitHub Issue #60。 |
