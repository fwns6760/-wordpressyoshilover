...
| **YouTube caption short quote summary** | `385-INGEST-youtube-caption-short-quote-summary.md` | LIVE_DEPLOYED_OBSERVE。user GO。YouTube 字幕を長い 600 字 1 block ではなく、LLM なし / 推測なしで短い引用 + 要点表示に寄せる。実装: caption text を deterministic に文分割し、最大2 quote + 最大3要点 bullet に整形。推測補完なし、HTML escape、出典 / embed 維持。follow-up repo fix: 取得材料 window を 600→1500 chars に拡大、表示 cap は維持。tests 63 OK、compileall / py_compile / AST / diff-check OK。Cloud Build `0752f06d` SUCCESS、fetcher rev `yoshilover-fetcher-00438-k8l` 100%、`/health` OK、新 revision ERROR 0。次は 1500 版 deploy と自然 fire で quote_count / summary_count evidence 待ち。env / Secret / Scheduler / RUN_DRAFT_ONLY / WP既存記事 / X / frontend は不可触。GitHub Issue #60。 |
