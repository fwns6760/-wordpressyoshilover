...
|---|---|---|
| **X-POST hallucination 防止 9-axis pack (2026-05-20、 DRAFT user GO 受領)** | `doc/active/414-X-POST-HALLUCINATION-PREVENTION-9AXIS.md` | user 報告 2026-05-20 chat hallucination 事例 3 件 (岸田 28位 = 実在しない順位 / 山瀬 = 2軍中心が 1軍 ranking 混入 / 試合中で古い data 拾い) を 9 axis で fix。 axis 1-7 + 9 = `src/x_post_branding_gen.py` (順位 forbidden regex / 数値 whitelist / 率系 regex / prompt bad example / temperature 0.4 / published_date 7日超 drop / drop log 構造化 / persona=kandume で当日 only)、 axis 8 = `src/analysis/active_roster_filter.py` 新規 + `pick_candidates` + ranking aggregate に直近 14 日 3 試合 1軍出場 filter。 並走 agent (403 chain `ranking_article_publisher.py`) と衝突 risk、 commit 直列必須。 cost ¥0/post 維持。 spec 382 hard rule 不変。 411 (`3dd89e5`) の brand post 2 persona base 上に積む精度強化。 |
