...
|---|---|---|
| **X-POST branding voice persona (2026-05-20 chat 起票、 DRAFT user GO 待ち)** | `doc/active/411-X-POST-BRANDING-VOICE-PERSONA.md` | user 仕様 lock: Tavily で候補 URL 拾う / 公式・NPB・球団・主要スポーツ紙優先 / Tavily answer 不使用、 URL 本文・媒体名・日付見る / 野球観戦の記事 / フーガ風 (長文分析) + 缶詰風 (試合中実況) 2 voice persona / 観戦ツイートは試合日 18時以降のみ。 392 (CLOSED) Gemma 4 + Tavily 基盤を拡張、 `_TAVILY_INCLUDE_DOMAINS` 2→9、 `_format_tavily_context` に published_date 注入、 prompt 2 persona split、 `is_giants_game_day` (insight.db games table) + `select_branding_persona` (18-21時 + 試合日 = 缶詰、 他 = フーガ)。 cost ¥0/post 維持。 並走: 394 active P0 hallucination fix 同 file commit 直列。 |
