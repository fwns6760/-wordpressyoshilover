...
| **source body excerpt share UI strip** | `400-QA-source-body-excerpt-share-ui-strip.md` | LIVE_DEPLOYED_VERIFIED。user 報告 (2026-05-20)「post 69888 の引用の部分にポスト・送る・シェア・ブックマークなど記事以外の情報がある、恒久的に、日本テレビ以外でも」。原因: `src/source_article_body_extractor.py` L204-219 の `_BOILERPLATE_LINES` set に share UI / SNS ボタンの label が入っておらず、React-rendered ニュースサイト (NTV / `news.ntv.co.jp` 等) が記事 container 内に sibling `<p>` として emit する `ポスト` / `送る` / `シェア` / `ブックマーク` / `URLをコピー` / `スポーツ` (breadcrumb) が `_strip_html_to_plain` の exact-line match を抜けて本文抜粋 (`📖 本文抜粋` block) に流入していた。narrow fix: `_BOILERPLATE_LINES` set に 23 ラベル追加 (スポーツ / ポスト / ツイート / 送る / シェア / ブックマーク / コピー / URLをコピー / リンクをコピー / クリップボードにコピー / クリップボードにコピーしました / シェアする / 保存 / 保存する / もっと見る / いいね / メール / 印刷 / LINEで送る / Facebookでシェア / Xでシェア / Twitterでシェア / はてブ / Pocket)。ロジック不変、exact-line match なので `ポストシーズン` / `メールマガジン` / `シェアを伸ばす` のような prose 内 substring は誤剥離しない。tests/test_source_article_body_extractor.py に NTV 風 fixture + false-positive 防止 fixture の 2 case 追加、pytest 46 + manual_intake 93 = 計 139 passed (regression 0)。commit `512dbe5`、Cloud Build `86d91583-a306-4f9d-b884-9de656a1b830` (manual-intake) SUCCESS 80s + `b93eac22-08f0-48a5-9afb-8e06e8601648` (fetcher) SUCCESS 94s、両 image tag `400-share-ui-strip-512dbe5`、revision `manual-intake-service-00086-x62` 100% + `yoshilover-fetcher-00453-gnt` 100%、両 `/health` OK。live verify (2026-05-20、新 revision deploy 後): live NTV HTML を `extract_article_body_excerpt` に通して share UI 7 token (ポスト / 送る / シェア / ブックマーク / URLをコピー / スポーツ / クリップボード) 全部 no_leak、本文 prose (リード「◇MLB ナショナルズ9-6メッツ...」 / ウッド選手描写 / グリフィン投手描写) は維持、新規 draft post_id `69899` を draft mode で作成成功。既存 post 69888 は retroactive cleanup しない(本 fix は今後生成 draft / publish のみに効く、user 判断で手動編集 or 再生成)。env / Secret / Scheduler / RUN_DRAFT_ONLY / WP既存記事 / X / publish-notice / x-post-mail-lane / frontend / 自動 RSS / tag scrape path 全部不変。残課題: 1 行目に photo credit 行「(写真：AP/アフロ)」が残る件は scope 別 (必要なら 401 / 402 として narrow に切り出し)。 |
