...
| **branding radar fresh news X Search** | `382-MKT-brand-radar-x-search-fresh-news.md` | user 要望「巨人だから新鮮なニュースが良い」「ヨシラバーのポストを見たいと思わせる」。調査結果: local `hermes` command は未導入、ただし `yoshilover-grok-api-key` secret は存在し、`yoshilover-fetcher` には `GROK_API_KEY` が already wired。repo には `x_search` を直接使う xAI Responses API 実装が既にある。PC off 前提なら Hermes local 常駐ではなく GCP Cloud Run Job + xAI API key が実装本線。status `BLOCKED_USER`: user が GCP direct xAI API / local Hermes smoke / article-only のどれで進めるか、x_search daily cap、new Job vs x-post-mail replace を決める。GitHub Issue #56。 |
| **fan voice X Search precision** | `383-QA-fan-voice-x-search-precision.md` | user 要望「本文の皆の声も同じ仕様で X Search で精度を上げたい」。現行 `💬 ファンの声（Xより）` は主に `fetch_fan_reactions_from_yahoo()` の Yahoo realtime path。既存 repo には xAI `x_search` 実装と `GROK_API_KEY` wiring があるため、382 と同じく GCP direct xAI API を本線、Hermes Agent は local smoke / fallback とする。status `BLOCKED_USER`: ticket-only request、実装は user GO 待ち。GitHub Issue #57。 |
...
- **owner**: Codex / **lane**: A
- **doc_path**: `doc/waiting/382-MKT-brand-radar-x-search-fresh-news.md`
...
- **調査証拠**: local `hermes` command は未導入。Secret Manager には `yoshilover-grok-api-key` が存在し、`yoshilover-fetcher` Cloud Run service には `GROK_API_KEY` として wired 済み。`x-post-mail-lane` Job には `GROK_API_KEY` は未設定。repo code には `src/rss_fetcher.py` / `src/x_post_generator.py` に `https://api.x.ai/v1/responses` + `tools: [{"type": "x_search"}]` の既存実装がある。
- **判断**: PC off 前提なら Hermes local 常駐ではなく、GCP Cloud Run Job + xAI API key path が実装本線。Hermes OAuth は local smoke / fallback 扱い。
- **proposal**: `brand-radar` pipeline を新設し、0-6h / 6-24h の新鮮な巨人記事を主役に、上位 topic だけ xAI `x_search` で反応証拠を取り、`ヨシラバー投稿企画案｜巨人ニュース鮮度レーダー` mail を送る。データは補助材料に下げる。
- **cost guard**: 初期案は max topics/run=3、max x_search/run=3、max runs/day=5、max x_search/day=15。mail に call count / cap / provider errors / skipped reasons を出す。
- **blocked_by**: user decision。A: GCP direct xAI API key、B: local Hermes smoke first、C: article-only radar。あわせて daily x_search cap と new Job / x-post-mail replace の判断が必要。
...
- **判断**: 382 と同じ。GCP direct xAI API + `x_search` が本線。Hermes Agent は local/manual smoke または fallback であり、本番 runtime の必須依存にしない。
