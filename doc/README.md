...
| **source body excerpt share UI strip** | `400-QA-source-body-excerpt-share-ui-strip.md` | READY_FOR_IMPL。user 報告 (2026-05-20)「post 69888 の引用の部分にポスト・送る・シェア・ブックマークなど記事以外の情報がある、恒久的に、日本テレビ以外でも」。原因: `src/source_article_body_extractor.py` L204-219 の `_BOILERPLATE_LINES` set に share UI / SNS ボタンの label が入っておらず、React-rendered ニュースサイト (NTV / `news.ntv.co.jp` 等) が記事 container 内に sibling `<p>` として emit する `ポスト` / `送る` / `シェア` / `ブックマーク` / `URLをコピー` / `スポーツ` (breadcrumb) が `_strip_html_to_plain` の exact-line match を抜けて本文抜粋 (`📖 本文抜粋` block) に流入していた。narrow fix: `_BOILERPLATE_LINES` set に 23 ラベル追加 (スポーツ / ポスト / ツイート / 送る / シェア / ブックマーク / コピー / URLをコピー / リンクをコピー / クリップボードにコピー / クリップボードにコピーしました / シェアする / 保存 / 保存する / もっと見る / いいね / メール / 印刷 / LINEで送る / Facebookでシェア / Xでシェア / Twitterでシェア / はてブ / Pocket)。ロジック不変、exact-line match なので `ポストシーズン` / `メールマガジン` / `シェアを伸ばす` のような prose 内 substring は誤剥離しない。tests/test_source_article_body_extractor.py に NTV 風 fixture + false-positive 防止 fixture の 2 case 追加、pytest 46 passed + manual_intake 93 passed = 計 139 passed (regression 0)。既存 post 69888 は retroactive cleanup しない(本 fix は今後生成される draft / publish のみに効く)。env / Secret / Scheduler / RUN_DRAFT_ONLY / WP既存記事 / X / publish-notice / x-post-mail-lane / frontend / 自動 RSS / tag scrape path 全部不可触。次 step: commit + Cloud Build (manual-intake-service + yoshilover-fetcher 両方、同 extractor 使用) → revision flip → NTV URL を draft mode で再投入 → 本文抜粋に share UI が残ってないことを `<p>ポスト</p>` 等の grep で verify。 |
