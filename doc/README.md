...
- **判断**: PC off 前提なら Hermes local 常駐ではなく、GCP Cloud Run Job + xAI API key path が実装本線。Hermes OAuth は local smoke / fallback 扱い。`@yoshilover6760` Premium/license が必要な場合、HTTP 401/403 は `x_search_auth_required` として明示する。
- **implementation**: `src/brand_radar.py` / `src/tools/run_brand_radar_mail.py` / `tests/test_brand_radar.py` を追加。fresh article collector、xAI Responses API `x_search` client、3候補 mail composer、dry-run default CLI、evidence/cost/skip logging を実装。
- **verification**: py_compile PASS、`python3 -m unittest tests.test_brand_radar` 13 tests OK。ネットワーク承認 source dry-run: candidates_sent=1、x_search_calls_used=0、DeNA-looking general RSS は `non_giants_topic` で reject、Full-Count 巨人 坂本候補のみ残存。
- **cost guard**: default max topics/run=3、max x_search/run=3、timeout=30s、provider error 継続時は run 内 disable。CLI `--x-search-cap` で下げられる。
- **blocked_by**: live deploy / env / Scheduler decision。new `brand-radar` Job or existing `x-post-mail-lane` replacement、`GROK_API_KEY` runtime wiring、mail cadence、daily x_search cap。
