...
| **INSIGHT lane (342 → 343 prerequisite chain)** | `342-INSIGHT-data-driven-ranking-auto-publish.md`, `343-INSIGHT-007-data-population-audit-and-backfill.md` | 342 = data-driven 定期 ranking 自動 publish 基盤(B 案: 設計 + 初版 4 候補 + 拡張 framework)、Phase 0 audit + Phase 1 spec done(commit `fc20b34` / `1058845`)。production DB GCS pull で INSIGHT-007 data ほぼ空発見(`teams: 0` / `players: 0` / `advanced_metric_snapshots: 0` / `games: 11` 行 2 日分)→ 342 impl は **343 prerequisite 待ち blocked**。343 = INSIGHT-007 nightly job が `teams` / `players` / `advanced_metric_snapshots` を populate していない原因 audit + backfill(DRAFT、user GO 待ち、3 source 採番 verify 完了)。GH Issue #21(342) + 343 起票予定。 |
