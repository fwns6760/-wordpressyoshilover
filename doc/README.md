...
| **OB YouTube differentiation backlog** | `317-QA-ob-youtube-review-only-intake.md` | PARKED / 後でやる。2026-05-20 user 判断「巨人OBなら差別化図るため記事にしてもいいかも」。2026-05-19 JST は YouTube下書き化が `post_id=69718` の1本のみで、OB候補は多くが `youtube_title_filter_skip reason=no_match` または `stale_rss_entry` で落ちた。後続実装時は公式YouTube 395 と分け、OB / 非公式は relevance gate を維持、巨人文脈が明確なものだけ `OB・解説者` の review-only / draft-only 候補へ流す。初期上限 1日1〜2本、env / Secret / Scheduler / YouTube Data API / WP既存記事 / X は不可触。GitHub Issue #76。 |
...
| **manual-intake React Helmet meta extract** | `399-INGEST-manual-intake-react-helmet-meta-extract.md` | READY_FOR_IMPL。user 報告 (2026-05-20)「`https://manual-intake-service-.../` で `https://news.ntv.co.jp/category/sports/5b63a75714734548999c19fbf038d767` が記事化できない」。原因は `src/tools/manual_intake.py` L187-198 `_META_PATTERNS` の regex が `<meta\s+property=` を前提にしており、NTV (React Helmet) の `<meta data-react-helmet="true" property="og:title" ...>` 形にマッチしないこと。`<title data-react-helmet="true">...</title>` も fallback regex `<title>([^<]+)</title>` (L399) に不発で、結果 `_parse_og_meta` が空 dict を返し `missing_title_or_summary` で 400。再現済: `python -c "from src.tools.manual_intake import _parse_og_meta; print(_parse_og_meta(open('/tmp/ntv.html').read()))"` → `{'title': '', 'summary': '', 'image': ''}`。narrow fix: regex を `<meta\b[^>]*\sproperty=...[^>]*\scontent=...` 形に書き換えて属性順非依存にし、title fallback も `<title\b[^>]*>` で属性付きを許容する。`_META_PATTERNS` 11 entry の付け替えと title fallback 1 行のみ。tests/test_manual_intake.py に react-helmet 形 + 既存 plain HTML regression の 3 case 追加。不可触: `_extract_context_matched_primary_og_meta` / `_fetch_news_meta` 本体 / 自動 RSS / tag scrape path / 各 source 専用 extractor / env / Secret / Scheduler / RUN_DRAFT_ONLY / WP既存記事 / X / publish-notice / x-post-mail-lane / frontend。 |
