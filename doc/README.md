...
| **YouTube source articleize fix** | `383-INGEST-youtube-source-articleize-fix.md` | 344 の YouTube 記事が出ない件。root cause は YouTube source が `media_quote_only` のため `media_quote_pool` 追加後に articleize path へ進まず、title filter / caption / `【YouTube】` prefix に到達していなかったこと。repo fix 済み、tests green、deploy / natural fire observation 待ち。Scheduler / env / Secret / WP既存記事 / X / frontend は未変更。GitHub Issue #58。 |
