...
| **Official YouTube titleless intake** | `395-INGEST-official-youtube-titleless-intake.md` | IN_FLIGHT。user 要望「YouTube の取り込みも」。383 で articleize path は復旧したが、巨人公式 YouTube の `小林の肩 vs 朝井の声` が title keyword 弱く `youtube_title_filter_skip reason=no_match` で落ちていたため、`official_video_source` ロールだけ titleless / weak-title pass を追加。非公式 / OB / candidate YouTube は既存 title filter 維持。local 59 tests + compileall / py_compile / AST / diff-check green。deploy / natural fire evidence 待ち。env / Secret / Scheduler / RUN_DRAFT_ONLY / WP既存記事 / X / frontend は不可触。GitHub Issue #72。 |
