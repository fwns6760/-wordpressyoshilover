...
| **quality nikkan-gendai source excerpt (IN_FLIGHT 2026-05-21)** | `doc/active/418-QA-nikkan-gendai-source-body-excerpt.md` | post `70027` / `nikkan-gendai.com/articles/view/sports/387913` で `extractor_len=0` → `meta_len=55` fallback になり引用が短い問題。共通 extractor に `www.nikkan-gendai.com` 専用 fallback を追加し、manual-intake-service / yoshilover-fetcher 両方へ deploy 対象。既存 post 70027 は retroactive 修正しない。GitHub Issue は api.github.com 接続不可で PENDING。 |
