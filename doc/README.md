...
| **X post branding LLM Phase 2 (Gemma 4 + Tavily REST)** | `392-x-post-branding-mcp-phase2.md` | PLANNING / user GO 後着手中。 391 Phase 1 CLI を本番 x-post-mail-lane に組み込む。 既存 template-based builder は LLM 生成 (Gemma 4 31B + Tavily HTTP REST + 任意 DB 参照) で置き換え。 mail format / Scheduler / Dockerfile / recipients は **既存 382 完全流用** (Dockerfile 不変、 Node 追加せず HTTP REST direct で回避策採用)。 spec 382 hard rule (URL / hashtag / 未検証数字 / 引用 / 媒体名 禁止 / yoshilover framing / 巨人 specific) 継承。 0 ドル制約 hard (Gemini API free / Tavily 150-450 credits/月 想定)。 env flag `X_POST_MAIL_GEMMA_GEN_ENABLED` default OFF で gating、 rollback 余地。 fault tolerance: Gemma 例外時 None 返却 + 既存 mail 送信続行。 私が 382 dirty で残してた multi-source shape B は本 ticket で supersede。 GitHub Issue #67。 |
