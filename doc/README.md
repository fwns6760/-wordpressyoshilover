...
|---|---|---|
| **⑥ 現役vsレジェンド対比 = OB年度別データ前提 (BLOCKED_USER 2026-06-04)** | `doc/waiting/471-DATA-ARTICLE-legend-comparison-data-prereq.md` | 設計 data-articles-no1 の最後の1項目 ⑥。OB 684 の年度別データ不在が唯一のブロッカー(`ob_legends_full`=career総計のみ/npb_id無/birth 56名欠損、実測)。着手手順=案A(Wikipedia年度別表を 21名 ground-truth で累計=総計検算 PoC)→ 成功なら 684拡張(別GCS `ob_career_yearly.json` 日次ingest)→ ⑥実装(env gate + draft, title case F)。fact精度critical(age計算誤り=致命的NG)+ 大scope のため **着手 GO は user 判断**。PoC は read-only(WP書込なし)で低リスク。仕様 §10 に解除手順記載。**Phase 0/1/2 の他項目(③②④⑤/歴代タブ/記録室)は全 LIVE**。 |
| **SNS リアルタイム話題 (巨人 1軍/2軍/3軍) daily aggregation (READY 2026-05-28 PM)** | `doc/active/445-SNS-realtime-topic-daily.md` / GH Issue #114 | Yahoo リアルタイム検索の巨人専門版を、 既存 RSSHub + 既存 Scheduler + 既存 fetcher pipeline に相乗りで実装 (==追加コスト ¥0==)。 source = 巨人専門 / 球団公式 X 4 account (`yomiuri_giants` / `TokyoGiants` / `hochi_giants` / `Sanspo_Giants`)、 RSSHub `https://rsshub-n5hunzkyna-an.a.run.app/twitter/user/{handle}` 経由。 1 日 4 fire (10/13/17/21 JST、 既存 `giants-weekday-daytime` / `giants-realtime-trigger` 相乗り)、 WP slug = `giants-sns-realtime-{YYYY-MM-DD}` で 1 日 1 URL upsert。 記事構造 = トレンド section (最上部、 全 136 名 aliases で言及回数 count、 上位 15 名 tag chip) + 一軍 / 二軍 / 三軍 section (oEmbed embed、 各 ≤ 5 件)。 分類 = 三軍 (`role=='ikusei'` or `育成/三軍/3軍`) / 二軍 (`ファーム/二軍/イースタン` or roster position `二軍/ファーム`) / 一軍 (default)。 監督・コーチ言及は Phase 1 で一軍配置を仮定。 新 Scheduler / Job / Dockerfile / cloudbuild は 作らない、 既存 `yoshilover-fetcher` の hourly run に hook。 spec: `mkdocs_docs/spec/sns-realtime-topic.md` (mkdocs nav 追加済、 `127.0.0.1:8000` mkdocs serve で preview)。 |
| **GCP cost safe cleanup (425 executed / 426 preflight STOP / 427 REPO_IMPL_TESTED 2026-05-23)** | `doc/done/2026-05/425-OPS-gcp-expired-one-time-scheduler-cleanup.md` / `doc/done/2026-07/426-OPS-artifact-registry-safe-cleanup-preflight.md` / `doc/active/427-OPS-publish-notice-skip-disabled-review-state-fetch.md` | parent GH Issue #97。425 は過去日 one-time Scheduler 2 件 (`publish-notice-extra-20260517-game`, `giants-game-extra-20260517-1400-1700`) を削除済み、Scheduler count `ENABLED 38 / PAUSED 18` → `ENABLED 36 / PAUSED 18`、想定削減は約 $0.20/month。426 は Artifact Registry `yoshilover` 約 98-103GB の安全 cleanup 前監査で、`publish-notice:prefilter-a32bd79` が live image だが cleanup keep tag list に入っていないため STOP。427 は Scheduler 間引きなしで `publish-notice` の不要 state fetch を省く repo narrow fix 実装 + tests 済み。Artifact delete / cleanup policy update / Cloud Run deploy は未実施。 |
| **XPOST branding requirements v2 (USER_HEARING_CAPTURED 2026-05-23)** | `doc/active/428-XPOST-branding-requirements-v2.md` / GH Issue #98 | ヨシラバーの巨人ファンインフルエンサー強化を目的に、X に実際に出す `post_text` の要件定義を仕切り直す。user hearing を反映し、Source A は「データに強い巨人ファン + 試合中の空気を拾う観戦仲間」のヨシラバー voice として、RSS 要約ではなく 180-280 字 / 2-3 行 / fact 1 + 観戦感 1 + ファン感情 1 / 次に見たい点へ着地する観戦ポストに再定義。Source B(DB ranking 418 case B) / Source C(specialized DB slices) は DB 表ポストとして Gemini 上書き禁止。415 の左右投手は DB がある例であり、左右投手に固定しない。優先はファンが欲しいデータポスト。同一選手の重複回避 / player rotation も要件化。記事作成 policy 423 / metric whitelist / dedup / diversity rule は article version の材料として使い、記事 title / 本文の短縮ではなく X 用 post version の `post_text` へ変換する。Tavily REST も禁止。実装 / deploy / env / Scheduler / WP / X live post は不可触。 |
| **XPOST branding v2 child tickets (PARTIAL_REPO_IMPL_TESTED 2026-05-23)** | `doc/active/429-XPOST-source-b-db-ranking-fixed-table-player-rotation.md` (GH #99) / `doc/active/430-XPOST-source-a-yoshilover-voice-validator.md` (GH #100) / `doc/active/431-XPOST-source-c-specialized-db-slices-fan-demand.md` (GH #101) / `doc/active/432-XPOST-source-mix-visibility-skip-reasons.md` (GH #102) / `doc/active/433-XPOST-post-text-regression-tests.md` (GH #103) / `doc/active/434-XPOST-anomaly-detection-phase1-flag-only.md` (GH #104) | 初期実装済み。433=`post_text`/X intent URL decode regression、432=mail text/html の Source A/B/C 件数 + flags 表示、434=deterministic flag-only、429=Source B DB表を final `post_text` に維持、430=Source A fallback / comment×DB を 180-280字3行化、431=Source C sample 条件初期 flag。未完: player rotation 新規強化、generation stage skip reason log、Gemini prompt 本体、Source C generator / fan-demand scoring。deploy / env / Scheduler / WP / X live post は不可触のまま。 |
| **XPOST branding v2 safe implementation design (LIVE_IMAGE_UPDATED 2026-05-23)** | `doc/active/435-XPOST-branding-v2-safe-implementation-design.md` / GH #105 | user GO 後、実装を `src/x_post_mail_lane.py` / `tests/test_x_post_mail.py` に限定して実施。`post_text` / X intent URL を正本にし、Source B表維持、Source summary、flag-only、Source A fallback voice、Source C初期flagsを追加。Cloud Build `b462008a-041d-41c9-8b1a-3a3e5f3540d8` SUCCESS、image `x-post-mail-lane:428-branding-v2-b9973f6` digest `sha256:507237...`、Cloud Run Job `x-post-mail-lane` generation `67`。Scheduler / env / Secret / WP / X live post は未変更、追加メール回避で手動 execute 未実施。 |
| **XPOST player history 24h → 168h (REPO_IMPL_TESTED 2026-05-28)** | `doc/active/441-XPOST-player-history-7d-window.md` / GH PENDING | user 「大城、 マルティネスなどかなりデル」 → 24h rolling window では昨日同時刻に出た player が今日同時刻に復活する構造 + `_load_recent_dedup_records` が today+yesterday 2 日分しか blob 読まず実効 ~48h 頭打ち + relaxed history fallback (`run_x_post_mail.py:1665-1690`) で 0 件時に history 無視リトライ、 の 3 重で症状発生。 fix: lookback default 24 → 168、 blob loop を `ceil(hours/24)+1` 日分に拡張、 env `X_POST_MAIL_PLAYER_HISTORY_HOURS` で上書き可、 relaxed fallback 削除 (0 件のままなら mail skip)。 `dedup_set` combo signature 系 fallback は player history 維持なので残す。 134 tests passed、 deploy + Cloud Run Job update 待ち。 |
| **XPOST Top10 data diversity / farm separation / sample gate (LIVE_IMAGE_UPDATED 2026-05-23)** | `doc/active/436-XPOST-top10-data-diversity-farm-sample-gate.md` / GH PENDING | 【Xポスト案 8件】のDB Top10表で、同じ選手・同じmetric・同じperiod・同じ表種別が続く問題を、候補metadata / 1軍・2軍・3軍分離 / sample gate / duplicate flags で初期実装。Top10表は維持。2軍/3軍は通常X候補に混ぜない。短すぎるrate系 row は原則X候補から外し、候補が薄くなる場合だけ `参考値・通常目安=規定打席/投球回N以上` と明記してfallback採用する。`直近5試合` 打者は表示thresholdを `規定打席10以上` に引き上げ。follow-upで投手rateの表示を `規定投球回N以上` に統一し、候補不足時でも24h重複選手を戻すfallbackを廃止。tests: targeted 70 passed + follow-up 37 passed、compile/AST OK。full `tests/test_x_post_mail.py` はtimeoutで未完走。Cloud Build `47b155de-8e7c-4cdb-8d4b-23f803329cf2` / `bb4f7c63-535e-447f-b2a4-0ea200586c00` SUCCESS、image `x-post-mail-lane:436-dedup-unit-fix-20260523-2122` digest `sha256:5f6d72...`、Cloud Run Job `x-post-mail-lane` generation `69`。env / Secret / Scheduler / WP / X live post は未変更、手動 execute 未実施。 |
| **X-post comment 候補 画像添付 brand_opinion + brand_quote (IN_FLIGHT, Phase 1+2 deploy 済 2026-05-28)** | `doc/active/438-XPOST-brand-opinion-and-quote-images.md` / GH PENDING | x-post-mail-lane の comment 系候補 (GEMMA_BRANDING / build_x_post_from_article_info) に **毎時 fire で image を毎回添付**、 新規 Cloud Scheduler / Cloud Run job 一切作らない (¥10/月/job 削減 lock 維持)。 2 pattern: A=brand_opinion (反応元 og:image + ヨシラバー voice text overlay)、 B=brand_quote (引用元 og:image + 「発言者名「literal 60-180 字長 quote」」 overlay)。 共通: source 画像 native aspect / 強制 crop なし、 画像内 brand mark / 引用元 焼かない、 出典は alt text (5/27 `bf56300` で削除、 出典担保は voice + 写真自体に委ねる)。 post text の hard rule (URL / ハッシュタグ / 媒体名 含めない) 維持。 prerequisite=5/27 commit `776252d` (監督・コーチ member gate 拡張)。 **実装 commit 11 本 (5/27-5/28)**: Phase 1=`b63888c` / Phase 2=`6a59de6` / 後続 fix=`bdcba6a` `cae5066` `5a0729b` `ea23382` `bf56300` `38b0085` `26b95db` (speaker proximity / URL 漏出 iOS+Android / 出典削除 / Pattern B 成立率改善) / docs=`3184a91` (mkdocs spec)。 next: 24h `gcloud billing` 実測 verify + 実 X 投稿で Pattern A/B 比率観察 → 問題なければ CLOSED + `doc/done/2026-05/` 移動。 deploy hygiene = .gcloudignore + feat/377 起点 + image tag に short sha。 |
| **WP eyecatch + X-post SVG 画像 動的生成 (READY 2026-05-25 PM scope 拡張)** | `doc/active/437-WP-XPOST-eyecatch-svg-dynamic.md` / GH #112 | 2026-05-25 PM scope 拡張: 3 → 12 style + format auto-routing + quality gate 10 項目 + 4 phase 構成。design sample 12 + variant (`sample9b_3crown`) を user desktop に保存済 (brand: 白 bg / vivid orange `#FF6F00` + gradient / 黒 stripe / 金 ★ / hook line / drop shadow / tabular nums)。Phase 1 = `ranking_article_publisher` で sample1 (ranking) のみ production quality release、 commit 3 本直列 (A: cairosvg + template + generator + tests / B: publisher 統合 + wp_client.upload_media / C: Dockerfile cairo + canary deploy)。Phase 2 = sample2/4/6/9/9b/10 + `x_post_image_router.py` (crown_count / player_count routing)。Phase 3 = 残り 6 + GCS cache。Phase 4 = X media attach + monitoring + A/B (user GO 後)。コスト見積 ¥0/月 (Cloud Run free tier 0.8% / GCS 9% / Network 45%、 LLM 不使用)。post-deploy 24h `gcloud billing` 実測 verify mandatory。env / Secret / Scheduler / WP 既存記事 / X live post は不可触。 |
| **X-post mail mode unification (IN_FLIGHT 2026-05-22)** | `doc/active/424-X-POST-MAIL-MODE-UNIFICATION.md` | 16:00 fire が 2 件 only (queue 417 直結のみ) になる捻れを是正、 1 fire = 1 mail = data ranking + Gemma branding + team roundup + queue 417 統合 path に戻す。 `--mode` 引数廃止、 旧 PAUSED schedule 5 本 delete、 queue 由来 candidate の unverified_numbers gate を弱 variant に分離。 |
| **X-post branding model swap (DONE_DEPLOYED 2026-05-22 evening)** | commit `4e8e038` / image `x-post-mail-lane:gemini-flash-lite-4e8e038` | `gemma-4-31b-it` → `gemini-3.1-flash-lite` 切替 (user 2026-05-22 evening 指示)。 free tier 1,500 RPD / volume 25 req/日 = 1.7%、 Cloud Run runtime 短縮 ¥1,000-1,500/月 節約見込。 24h billing verify pending。 |
| **データ publish ルール集約 SoT (LOCK 2026-05-22)** | `doc/active/423-DATA-PUBLISH-RULES-CONSOLIDATED.md` (GH Issue #96) | データ post の 9 領域 rule (metric whitelist / 期間 / threshold / title / dedup / draft status / mail / 関連表示 / site 方向性) を 1 file に集約。 metric ◯/× の SoT は `doc/reference/data-insight-metric-whitelist.md` を維持、 本 file は それ + 他 rule の集約。 5/22 lock した同日 hard block + player_daily_cap=2 + ERA 「低い順で」 削除 + 【まとめ】mail 停止 (env `DISABLE_BURST_SUMMARY_MAIL=1`) も記載。 個別 rule 変更時はこのファイル同 commit 更新する運用。 |
| **#95 / 417 ヨシラバー X-post branding 2 source mix + 試合中 15 分 schedule (DONE 2026-05-22、 close 候補)** | GH Issue #95 / `doc/active/417-X-POST-MAIL-HOCHI-PRIORITY-PIGGYBACK.md` | 4 scope 全完了: (1) Source A voice 統一 (ヨシラバー voice 180-280 字、 commit `9cc08b1` / image `yoshilover-voice-9cc08b1`)、 (2) Source B 維持 (418 case B format、 commit `6ab949f`)、 (3) schedule (`x-post-mail-flush 0 6-22 *` + `flush-game-1 15,30,45 19-20 *` + `flush-game-2 15,30,45 21 *`)。2026-05-23 22:01 JST に user request で 21:45 を追加、Job target/env/Secret/WP/X live post は不変、 (4) Tavily 0 化 (旧 5 cron PAUSE + 5/17 one-time cron DELETE、 log verify 0 hit 6:00 UTC 以降)。 user 承認後 close。 |
| **X-POST brand voice quality framework (CLOSED LIVE_DEPLOYED_VERIFIED 2026-05-21)** | `doc/done/2026-05/414-X-POST-BRAND-VOICE-QUALITY-FRAMEWORK.md` | 全 5 軸 A-E landed (A 型分離 `7bb8682` / B persona `3dd89e5` / C 9 axis `c0ac71e` + C8 `02e3bb7` / D 炎上 6 check `faa7226` / E 試合前 7 テーマ `b3c80b0` + caller wire `47c8961` + E3/E7 `4beef83`)。 image `x-post-mail-lane:414-e3e7-4beef83` gen 40 deploy 済、 2026-05-21 07:04 JST 自然 fire で axis C 動作確認 (`gemma_branding_drop` `unverified_numbers` ダルベック 「7」 drop)、 mail 8 candidates 送信成功。 GH Issue #90 CLOSED。 |
| **X-POST branding voice persona (CLOSED LIVE_DEPLOYED_VERIFIED 2026-05-21)** | `doc/done/2026-05/411-X-POST-BRANDING-VOICE-PERSONA.md` | commit `3dd89e5` landed + image `x-post-mail-lane:414-e3e7-4beef83` gen 40 deploy 済。 2026-05-21 07:04 JST 自然 fire で persona switch / 18時 gate (`fan_voice skip: not in fire window`) / Tavily 3 results / Gemma mail 8 candidates 動作確認、 `mail send result: status=sent`。 GH Issue #88 CLOSED。 |
| **quality nikkan-gendai source excerpt (CLOSED LIVE_DEPLOYED_VERIFIED 2026-05-21)** | `doc/done/2026-05/418-QA-nikkan-gendai-source-body-excerpt.md` | post `70027` / `nikkan-gendai.com/articles/view/sports/387913` で `extractor_len=0` → `meta_len=55` fallback になり引用が短い問題。共通 extractor に `www.nikkan-gendai.com` 専用 fallback を追加し、manual-intake-service `00094-wf2` / yoshilover-fetcher `00466-tjs` へ deploy 済。両 `/health` OK、新 revision ERROR 0。既存 post 70027 は retroactive 修正しない。GitHub Issue は api.github.com 接続不可で PENDING。 |
| **subtype OB + farm 2/3 split + WP front (2026-05-20、 全 Phase LIVE_DEPLOYED_OBSERVE)** | `doc/done/2026-05/407-SUBTYPE-ob-and-farm-split-master.md` / 408 / 409 / 410 | 親 407 で design lock + 子 408/409/410 全 Phase 完了。 408 Phase 1+2+3 (`215abbf`+`afdfd5c`+`1cb561c`): OB literal marker primary + name table 32 seed + 現巨人 role guard 13。 409 Phase 1 (`64125cb`): farm 2軍/3軍 分離 (`ENABLE_FARM_2GUN_3GUN_SPLIT` flag default OFF) + 4 新 subtype + backward-compat alias。 410 Phase 1+2 (`b46ffaa`+`d963d1d`): badge + 出典帯 builder + draft pipeline wire-in。 production deploy 2 回 (`00459-44z`/`00460-tng`、 両 /health 200)、 pytest 5404 → 5561 regression 0、 GH Issue #80/#82/#83/#84。 設計 anchor: `docs/handoff/session_logs/2026-05-20_ob_subtype_and_farm_split_design.md`。 残: 自然 fire 観察 / 409 enable (env、 §11) / WP CSS 追加 (`.nomotoke-subtype-badge--*`、 user 判断)。 |
...
| **quality farm/third postgame box guard** | `367-QA-farm-third-postgame-first-team-box-guard.md` | 68610 型。三軍 / 二軍の試合結果記事が `first` 誤判定で当日の一軍 NPB / Yahoo boxscore を取りに行く穴を、postgame 抽出器と rss_fetcher の二層 guard で止める。GitHub Issue #36。 |
| **quality X+Web quote dedup/title** | `368-QA-x-web-post-quote-dedupe-title.md` | 68628 / 68633 型。Xポスト + 引用記事を残す側にし、Web-only duplicate を同 fire で消費。X記事の関連記事は literal detail overlap 必須にし、title を Web title/summary で具体化する。GitHub Issue #37。 |
| **OB YouTube differentiation draft-only** | `doc/done/2026-05/317-QA-ob-youtube-review-only-intake.md` | CLOSED (2026-05-20; GitHub Issue #76 close済み)。巨人OB YouTube source を RSS 本線へ draft-only 接続済み。`giants_ob` source は weak title でも候補化し、`OB・解説者` category + `social_video_notice` body / YouTube embed で draft 作成、publish skip reason は `draft_only,youtube_review_source_draft_only`。quote-only guard により `youtube_review_notice` は字幕引用 block のみ、`要点` list なし。CTA / 音楽・拍手 / URL 系ノイズは引用候補から除外。current live image `402-meta-fallback-671df22` は 317 / 401 descendant のため live 維持。受け入れ観察は user 判断により別 ticket 化。 |
| **OB YouTube quote-only / 1200 excerpt deploy refresh** | `doc/done/2026-05/317-QA-ob-youtube-review-only-intake.md` / 401 runtime | LIVE_DEPLOYED_VERIFIED (2026-05-20)。`1fd6a6c` で `youtube_review_notice` は字幕引用 block のみ (`要点` list なし) にし、CTA / 音楽・拍手 / URL 系ノイズを引用候補から除外。関連/本文抜粋は `src/tools/manual_intake.py` の `SOURCE_BODY_EXCERPT_MAX_CHARS = 1200` と `src/rss_fetcher.py` の `extract_article_body_excerpt(..., max_chars=1200)` で manual-intake-service / fetcher 両方 1200 字に統一。baseline: `git diff --check` OK、py_compile OK、compileall OK、AST `618` OK、sandbox外 full pytest `5398 passed, 1 xfailed, 3 xpassed`。deploy: manual-intake-service revision `00088-vn8` image `401-excerpt-1200-1fd6a6c` generation `101/101` 100% / `/health` OK / ERROR 0。fetcher clean archive `d1c5d7a`、Cloud Build `f9799b40-89b7-400d-9b21-2ba972e80189` SUCCESS、image `401-excerpt-quote-d1c5d7a` digest `sha256:8dcae877...`、revision `00457-pcr` 100%、generation `597/597` -> `599/599`、`/health` OK / ERROR 0。final current: 後続 402 deploy で fetcher `00458-2dq` / manual `00089-xtf`、image `402-meta-fallback-671df22`、generation `600/600` / `102/102` に進行。`671df22` は `1fd6a6c` / `d1c5d7a` descendant のため、1200 字化と quote-only guard は live 維持。両 `/health` OK / ERROR 0。env / Secret / Scheduler / RUN_DRAFT_ONLY / WP既存記事 / X / manual `/run` fire は未変更。 |
| **quality short player event title** | `369-QA-short-player-event-title-quality.md` | 68539 / 68608 / 68795 型。`浦田俊輔、発言` / `浦田俊輔、安打` / `菅野智之「阿部さん」` のような短すぎる `選手名 + 薄いイベント語/短い引用` title を、同じX source本文内の literal な `2回適時打` / `3安打1打点猛打賞` / `日米通算150勝` 等で補強する。GitHub Issue #38。 |
...
| **Giants-only general source expansion** | `doc/done/2026-05/381-INGEST-giants-general-source-expansion.md` | user 要望「巨人だけ総合」「読売新聞/朝日新聞/毎日新聞/週刊ベースボール/一般誌」。commit `c791f52` 実装 + deploy 済。Full-Count feed 修正、ベースボールチャンネル、朝日/毎日スポーツRSS、週刊ベースボールONLINE、読売オンライン、日テレ、FRIDAY、Smart FLASH、週刊女性PRIME、文春、NEWSポストセブン、デイリー新潮、現代ビジネス、アサ芸を追加。x-post-mail fallback は tag_scrape 対応 + source limit 32 で全32 article sources 到達。manual-intake-service に source 候補 tab + `/source-candidates` を追加 deploy 済み、live 読売 endpoint `count=3`。一般誌/新聞は limited topic bypass のみ、full bypass / numeric fact / hard stop は不変。deploy evidence: x-post-mail build `a8a5ea80` SUCCESS / Job generation `22` Ready=True、fetcher build `2b60df25` SUCCESS / revision `yoshilover-fetcher-00431-vl9` 100% / `GET /health` OK、manual-intake build `9329593b` SUCCESS / revision `manual-intake-service-00084-bzv` 100%。Scheduler / env / Secret / WP / X は未変更、manual execute 未実行、post-update mail acceptance は次回自然 fire 待ち。GitHub Issue #55。 |
| **Yoshilover branding post planning mail** | `382-MKT-yoshilover-branding-post-planning-mail.md` | CLOSED。user 要望「目的はヨシラバーのブランディングポスト案」。既存 x-post-mail lane 内で、URLなし / hashtagなし / サイト誘導なしの編集用Xポスト案へ変更。未照合数値は本文から除外し、DB照合済み数値だけ許可。commit `4507257` + follow-up full regression `1013716`、Cloud Build `1629e082` SUCCESS、`x-post-mail-lane` generation 23 Ready=True、2026-05-19 07:00 JST 自然実行 `x-post-mail-lane-psp2s` SUCCESS / mail sent / ERROR 0。Scheduler / env / Secret / WP / X live post は不変。GitHub Issue #56。 |
| **YouTube source articleize fix** | `doc/done/2026-05/383-INGEST-youtube-source-articleize-fix.md` | CLOSED。344 の YouTube 記事が出ない件。root cause は YouTube source が `media_quote_only` のため `media_quote_pool` 追加後に articleize path へ進まなかったこと。repo fix + deploy 済み。Cloud Build `7ed5103a` SUCCESS、fetcher rev `yoshilover-fetcher-00436-7zq` 100%、`/health` OK、新 revision ERROR 0。2026-05-19 natural fire で `tag_page_entries_built source=youtube` と `youtube_title_filter_skip` を確認し、YouTube path log acceptance を満たしたため GitHub Issue #58 close。Scheduler / env / Secret / WP既存記事 / X / frontend は未変更。 |
| **YouTube caption draft expansion parent design** | `doc/done/2026-05/344-INGEST-youtube-caption-draft-expansion.md` | CLOSED / SUPERSEDED。広めの親設計は 383 / 385 / 395 / 398 / 317 の実装済み chain に分割。GitHub Issue #24 は既に close。受け入れ観察は user 判断により別 ticket 化。 |
| **Source expansion + publish-time fallback** | `doc/waiting/384-INGEST-source-expansion-with-publish-time-fallback.md` | DESIGN_LOCKED / READY_FOR_IMPL。user 要望「ソースをふやしたい、ガードに引っかからない」(2026-05-19 chat)。381 で追加した 16 family のうち meta なし媒体が `source_time_missing_review` review 落ち (5/19 9 時 cycle 52 件 evidence)。Phase 1 = 産経新聞 (meta `name` attribute) + 日刊SPA (meta `property` attribute) を tag_scrape 追加。Phase 2 = `tag_page_scraper.py` に publish-time fallback chain 追加 (name attribute / modified_time / URL date / body 内日付表記)、`_strict_source_time_review_required` の guard logic は変えず scraper 側で source_time を立てる設計で品質 gate 維持。Phase 3 = 中日新聞・中日スポーツ追加 (Phase 2 fallback の効果 evidence ベース判断)。Phase 4 = THE ANSWER (SPA 対応、別 ticket 化検討)。デグレ試験: 既存 16 family の挙動不変 regression、`STRICT_BREAKING_NEWS_THRESHOLDS` 閾値判定不変、scraper の `_is_ymd_within_window` で古い記事は弾く 2 重 guard。`_POST_GEN_VALIDATE_TRUSTED_FAMILIES` (full bypass) / numeric/fact validator / close_marker / placeholder_body / hard-stop は不可触。env / Secret / Scheduler / X / SNS / WP既存記事も不可触。GitHub Issue #59。 |
| **YouTube caption short quote summary** | `doc/done/2026-05/385-INGEST-youtube-caption-short-quote-summary.md` | CLOSED (2026-05-20; GitHub Issue #60 close済み)。YouTube 字幕を LLM なし / 推測なしで短い引用 + 要点表示に寄せ、取得材料 window は 600→1500 chars に拡大。tests 63 OK、Cloud Build `82719b1a` SUCCESS、fetcher rev `yoshilover-fetcher-00441-xj8` 100%、`/health` OK、新 revision ERROR 0。受け入れ観察は user 判断により別 ticket 化。 |
| **Official YouTube titleless intake** | `doc/done/2026-05/395-INGEST-official-youtube-titleless-intake.md` | CLOSED (2026-05-20; GitHub Issue #72 close済み)。巨人公式 YouTube の weak-title / titleless pass を `official_video_source` ロールに限定して追加し、非公式 / OB / candidate YouTube は既存 title filter 維持。commit `43b101a` + priority follow-up `ffdb668` deploy 済み。手動 run で公式YouTube post_id `69846` draft 作成確認。受け入れ観察は user 判断により別 ticket 化。 |
| **Media quote evaluation default guard** | `doc/done/2026-05/398-INGEST-media-quote-evaluation-default.md` | CLOSED (2026-05-20; GitHub Issue #73 close済み)。`media_quote_evaluation` / `media_quotes` safe default を各 entry 処理開始時に置く narrow fix。tests 27 OK、Cloud Build `14880fed-c77f-44a1-b168-b5705561bf12` SUCCESS、fetcher rev `yoshilover-fetcher-00452-glk` 100%、`/health` OK、新 revision ERROR 0。受け入れ観察は user 判断により別 ticket 化。 |
| **manual-intake React Helmet meta extract** | `doc/done/2026-05/399-INGEST-manual-intake-react-helmet-meta-extract.md` | LIVE_DEPLOYED_VERIFIED。user 報告 (2026-05-20)「`https://manual-intake-service-.../` で `https://news.ntv.co.jp/category/sports/5b63a75714734548999c19fbf038d767` が記事化できない」。原因は `src/tools/manual_intake.py` L187-198 `_META_PATTERNS` の regex が `<meta\s+property=` を前提にしており、NTV (React Helmet) の `<meta data-react-helmet="true" property="og:title" ...>` 形にマッチしないこと。`<title data-react-helmet="true">...</title>` も fallback regex `<title>([^<]+)</title>` (L399) に不発で、結果 `_parse_og_meta` が空 dict を返し `missing_title_or_summary` で 400。narrow fix: regex を `<meta\b[^>]*\sproperty=...[^>]*\scontent=...` 形に書き換えて属性順非依存にし、title fallback も `<title\b[^>]*>` で属性付きを許容。`_META_PATTERNS` 11 entry + title fallback 1 行。tests/test_manual_intake.py に `test_react_helmet_attribute_first_meta` / `test_react_helmet_title_tag_with_attribute` 2 case 追加、pytest 93 passed (regression 0)。commit `fbb388d`、Cloud Build `90d41bc9-b298-45d9-8a8e-7892b0a507ea` SUCCESS 1m16s、image `manual-intake-service:399-react-helmet-fbb388d` digest `sha256:61460a15820ac52dc4cb7c1df050c620da5100161097c50b705d6cecd3842f21`、revision `manual-intake-service-00085-s2m` 100%、`/health` OK、新 revision ERROR 0。live verify (2026-05-20): NTV URL dry-run で HTTP 200 / `ok: true` / `validation_ok: true` / title 完全抽出 / `category="OB・解説者"` / `subtype="general"` / `template_key="nomotoke_card_short_news_url_v1"` / `article_type_guess="試合速報"` / `category_ids=[667]`。不可触: `_extract_context_matched_primary_og_meta` / `_fetch_news_meta` 本体 / 自動 RSS / tag scrape path / 各 source 専用 extractor / env / Secret / Scheduler / RUN_DRAFT_ONLY / WP既存記事 / X / publish-notice / x-post-mail-lane / frontend。 |
| **source body excerpt 全文恒久 clean (share UI + photo credit + copyright + lead heading)** | `doc/done/2026-05/400-QA-source-body-excerpt-share-ui-strip.md` | LIVE_DEPLOYED_VERIFIED (拡張済 2026-05-20)。user 報告「post 69888 の引用にポスト・送る・シェア・ブックマーク等記事以外が混入、恒久対応、日テレ以外も」「全文恒久対応でないの?」「アプリ以外も対応」「引用文をよみやすく」を全面 fix。**Phase A (commit `512dbe5`)**: `src/source_article_body_extractor.py` L204-219 `_BOILERPLATE_LINES` set に share / SNS UI 23 ラベル追加。**Phase B (commit `0a29e7f`)**: `_PHOTO_CREDIT_LINE_RE` 追加 (行末に `[（(](写真|撮影|提供|画像|Photo|PHOTO)[：:＝=／/]?...[）)]` 含む caption metadata 行を drop) / `_COPYRIGHT_LINE_RE` 追加 (行頭 ©/Ⓒ/(c)/Copyright/COPYRIGHT/無断複写/無断転載/All Rights Reserved を drop) / `_HEADING_PREFIXES` に `◇ ▽ ◎ □` 追加 (NTV/朝日/報知 試合速報リードを heading 化、`.nomotoke-source-excerpt__heading` で読みやすく)、`_strip_html_to_plain` と `_is_noise_line` 両 path で同 regex を check。ロジック改変なし、exact-line match + 末尾 anchor 規律で prose 内 substring (ポストシーズン/メールマガジン/シェアを伸ばす) は誤剥離しない。tests/test_source_article_body_extractor.py に 5 case 追加 (share UI strip / false-positive 防止 / photo credit tail / copyright / diamond lead heading)、pytest 計 142 passed + manual_intake 93 = 全 235 passed (regression 0)。両 service 経路 (manual-intake-service + yoshilover-fetcher) は同じ `extract_article_body_excerpt` と `classify_excerpt_paragraph` を import (manual_intake.py:1440-1443 / rss_fetcher.py:20459-20460,27978) しているので **手動 app だけでなく自動 RSS 経路にも同等に効く**。**deploy chain**: Phase A Cloud Build `86d91583-a306-4f9d-b884-9de656a1b830` (manual-intake) + `b93eac22-08f0-48a5-9afb-8e06e8601648` (fetcher) 各 80s/94s SUCCESS → revision `manual-intake-service-00086-x62` + `yoshilover-fetcher-00453-gnt`。Phase B Cloud Build `0c5328d0-c4f0-41bf-b408-dfb3c0dee61c` (manual-intake, 81s) + `b10db4be-2616-4980-9c4c-5cfbbe6ac56d` (fetcher, 68s) 両 SUCCESS、image tag `400-ext-readable-0a29e7f`、revision `manual-intake-service-00087-rfv` 100% + `yoshilover-fetcher-00455-lcf` 100%、両 `/health` OK、新 revision ERROR 0。**live verify**: live NTV HTML を extractor に通して 9/9 noise token (ポスト/送る/シェア/ブックマーク/URLをコピー/スポーツ/写真：/撮影：/Copyright) no_leak、本文 prose (リード「◇MLB ナショナルズ9-6メッツ...」が heading 化、ウッド選手 / グリフィン投手 4 段落) 完全維持、photo caption 行 (写真：AP/アフロ) も完全消失、新 draft post_id `69899` 作成成功。既存 post 69888 は retroactive cleanup しない (user 判断で手動編集 or 再生成、`https://yoshilover.com/wp-admin/post.php?post=69888&action=edit`)。env / Secret / Scheduler / RUN_DRAFT_ONLY / WP既存記事 / X / publish-notice / x-post-mail-lane / frontend / tag scrape path 全部不変。 |
| **INSIGHT no-game-day normal exit** | `doc/done/2026-05/386-INSIGHT-no-game-day-normal-exit.md` | CLOSED。2026-05-18(月) 試合なしで `insight-nightly` が `auto_resolve_all_failed` / exit 2 になった件。`status=no_game_day` / exit 0 修正を deploy / live verify 済み。follow-up で no-game 日でも DATA-INSIGHT publish/mail が出る path を追加。commits `a852abf` / `8454a21` / `0d09c10`。evidence: `insight-nightly:386-no-game-publish-8454a21` Job generation `68`、execution `insight-nightly-j4sbm` 成功、`【巨人データ】` post IDs `69545` / `69546` / `69547` publish。`publish-notice:data-insight-mail-0d09c10` Job generation `107`、execution `publish-notice-gm5tb` 成功、summary `sent=3`、per-post sent `69545` / `69546` / `69547`。GitHub Issue #61。 |
...
| **X post branding LLM Phase 2 (Gemma 4 + Tavily REST)** | `doc/done/2026-05/392-x-post-branding-mcp-phase2.md` | LIVE_DEPLOYED_OBSERVE。 391 Phase 1 CLI を本番 x-post-mail-lane に組み込み済 (commit `a15d852`)。 22:30 fire 観察結果: Gemma 生成 2 件 (吉川尚輝 / 泉口友汰) は cheerleading 定型語 (ついに / 物語が / 核心に迫る) + Tavily 古い snippet (Wikipedia / 2024-2026年3月) の現在化 で出力品質低下。 同夜 tone tune (commit `414d411`) + same-day filter + 時間帯 tone hint (commit `1a55d59`) で改修。 image `392-tune-1a55d59` Job generation 28 deploy 済、 翌 07:00 JST am-1 fire で改修版観察予定。 env `X_POST_MAIL_GEMMA_GEN_ENABLED=1` (ON、 5 fire/日 × MAX=2)。 spec 382 hard rule (URL / hashtag / 未検証数字 / 引用 / 媒体名 禁止) 継承。 fault tolerance: silent skip on Tavily / Gemma error。 0 ドル維持 (Gemini API free / Tavily ~300/月)。 GitHub Issue #67。 |
| **Gemma branding DB fact line 注入 (当日試合 / 連勝 / player log)** | `394-x-post-gemma-db-fact-line.md` | LIVE_DEPLOYED_OBSERVE。 392 出力の hallucination 抑制と「当日具体数字」 punch up を目的に、 `insight.db` の今日試合 / player log / 直近 N 試合連勝記録を fact line 化して Gemma context に注入。 user 明示 (2026-05-19): 「データベースは当日のきろくがいい、 試合前と試合中」「勝ち負けは RSS で拾える?」 (= 既存 ETL `games.result` で取得済、 別途 RSS 不要)。 実装: `src/x_post_branding_gen.py` に `build_db_fact_line()` 追加 (read-only SELECT、 mutation なし、 例外時空 string)、 `src/tools/run_x_post_mail.py` の `_build_gemma_branding_candidates` に `db_path` 引数追加 (既存 download 済 cache 再利用、 追加 GCS access なし)。 fact line 形式: 「今日(YYYY-MM-DD) 巨人 vs OPP: GS-OS (勝利)」「PLAYER 打撃 (今日): X打数Y安打 Z打点」「PLAYER 投球 (今日): X.Y回 Z失点 K奪三振 (○)」「直近N試合: ○●○●○ (X勝Y敗Z分)」。 commit `ef1e756`、 tests 24/24 + 関連 126/126 pass (regression 0)。 image `394-db-fact-ef1e756` deploy 後 翌 07:00 fire で観察予定。 392 / 391 既存 commit は supersede しない、 機能拡張のみ。 |
| **price-neutral draft judgment mail** | `doc/done/2026-05/393-OPS-price-neutral-fast-draft-judgment-mail.md` | LIVE_DEPLOYED_OBSERVE。user GO (2026-05-19): Cloud Run / Scheduler 起動回数を増やさず、記事作成後に本文付き mail で公開判断したい。repo commits `a626bda` / `bd0bb07` / `b495f6a`。`publish_notice_email_sender` に本文抜粋・編集リンク・公開ボタン付き `judgment_batch` summary mode を追加し、大量時は Part 分割。`rss_fetcher` から draft 作成同一実行内で少数時は個別、多数時は Part 分割 mail を送る。fetcher inline sent marker は既存 GCS `publish_notice/queue.jsonl` に merge し、後続 `publish-notice` の再送を止める。deploy: publish-notice image `393-judgment-bd0bb07` (build `f22f5f1e...`, digest `sha256:22991db...`) Job generation `111`; fetcher image `393-inline-mail-b495f6a` (build `be7cbe88...`, digest `sha256:b979707...`) revision `00445-zm7` 100%、`/health` OK。env ON 確認済。価格据え置き cleanup として追加 Scheduler `publish-notice-peak-followup` は `PAUSED`。残 acceptance は次回自然 fetch の `fetcher_inline_draft_notice_result` と実 mail 本文確認。GitHub Issue #68。 |
| **X post gen MCP Tavily + Gemma 4 Phase 1** | `doc/waiting/391-x-post-gen-mcp-tavily-gemma4-phase1.md` | PLANNING / user GO 待ち (Markdown 提示済、 code 編集 stop)。 user 要望 (2026-05-19): 巨人関連を Tavily MCP (stdio 同梱、 `npx -y tavily-mcp@latest`) で web 検索 → Gemma 4 31B (Gemini API free tier) で X 投稿案を生成する CLI smoke を新設。 $0 制約 hard rule (Gemini API free / Tavily 1000 credits/月 / Cloud Run・Vertex AI 自前 host 禁止)。 Phase 1 scope = `src/x_post_gen_mcp.py` + `src/tools/run_x_post_gen_mcp.py` + `tests/test_x_post_gen_mcp.py` + `requirements.txt` に `google-genai` + `fastmcp` 追加。 出力 = stdout のみ、 mail / WP / X 連携は **Phase 2 (別 ticket)**。 必要 API: Gemini key + Tavily key、 両方無料 signup、 credit card 不要。 不可触: 382 改修中 `src/x_post_mail_lane.py` / 387 改修中 `src/analysis/` / automation / scheduler / env / secret / WP / X live posting / 既存 Cloud Run Job。 spec 382 hard rule (URL / hashtag / 未検証数字 / 引用 / 媒体名 禁止) 継承。 process compliance: 私が ticket 切らずに code 2 file (`src/x_post_gen_mcp.py` / `src/tools/run_x_post_gen_mcp.py`) を書いた事実を user が指摘 (2026-05-19)、 本 ticket は後付け正規化。 既に書いた 2 file は dirty として残置、 commit はまだ。 GitHub Issue #66。 |
...
- **方針**: UZR / 守備率 title は `{巨人選手名}の{守備位置}、巨人は{指標} {値}でセ・リーグN/6位（期間）` 型。本文ではセ・リーグ球団別 table を見せる。`## データ` / `## このデータについて` / ranking / 比較 section は table 必須、`## データ` の bullet list 回帰は quality gate で止める。
...
- **acceptance**: 2026-05-18 mail 型 fixture を赤で固定し、修正後は十分な代替 row がある時は同一 player 1 件まで、代替不足時も default 2 件までに抑える。既出 player が top Giants row の metric は次の Giants row へ差し替え可能なら差し替える。data candidates alone で distinct player が足りない場合は、source title / URL / excerpt / player detection evidence 付きの news/opinion fallback で補充してよい。source に無い quote / 数字 / 断定は作らない。skip / fallback は structured log に残す。24h combo dedup / 374 starvation fallback / lineup focus spread は維持する。Scheduler / env / Secret / WP / X / SNS / production DB は変更しない。
- **implementation**: `src/x_post_mail_lane.py` で same-player cap default 2、既出 player を避ける next Giants row 選択、duplicate backlog + cap log を追加。`src/tools/run_x_post_mail.py` で data candidates が不足する場合だけ public RSS/Atom の source title / URL / summary / roster alias 検出から news/opinion fallback を追加。mixed mail は `巨人Xポスト案` / `データ+ニュース意見` label にする。
- **verification**: `python3 -m py_compile src/x_post_mail_lane.py src/tools/run_x_post_mail.py tests/test_x_post_mail.py` PASS。`python3 -m pytest -q tests/test_x_post_mail.py` = `89 passed, 3 warnings in 1.62s`。`python3 -m unittest tests.test_x_post_mail` = `Ran 89 tests in 0.513s` / `OK`。
- **deploy**: 2026-05-18 12:52 JST、12:01 mail が旧 image だったため image only deploy。Cloud Build `ba632b1d-0bdd-4a09-b314-e7b8b3ea1c3e` SUCCESS、image `x-post-mail-lane:380-player-diversity-31aa4e7`、digest `sha256:64e78e6f799dedc16323c49df502dc2d07c4d0cbb545ab549f899368a9e9666c`、Cloud Run Job generation `20`。12:01 mail evidence: Gmail id `19e3907b7bdba8e5`、浦田俊輔 3/8、マルティネス 3/8、pre-deploy image `376-lineup-spread-e488052`。
...
- **owner**: Codex / **lane**: A
- **doc_path**: `doc/done/2026-05/382-MKT-yoshilover-branding-post-planning-mail.md`
...
- **implementation**: `src/x_post_mail_lane.py` / `src/tools/run_x_post_mail.py` / `tests/test_x_post_mail.py`。既存 x-post-mail lane 内で、URLなし / hashtagなし / `ヨシラバーで整理しました` なし / サイト誘導なしのブランディング候補へ変更。`src/brand_radar.py` / `src/tools/run_brand_radar_mail.py` / `tests/test_brand_radar.py` の repo-only collector 試作も保持。X live post / WP mutation はなし。
- **copy contract**: 1ポスト1論点。コメントは source に存在する短い表現だけ使う。DB照合済み数値だけ本文に出し、未照合数値は削除または一般化する。
- **numeric contract**: exact 数値は DB 照合済みだけ許可。未照合の RSS / comment / trend 数値は投稿本文に入れない。根拠欄に omitted / evidence を残す。
- **verified**: commit `4507257` + follow-up `1013716`。Cloud Build `1629e082-f0c6-4b99-a1c0-a53cd9fd3d1f` SUCCESS、image `x-post-mail-lane:382-branding-4507257` digest `sha256:fa1981...`、Cloud Run Job generation 23 Ready=True。2026-05-19 07:00 JST 自然実行 `x-post-mail-lane-psp2s` SUCCESS、`Composing mail with 10 candidates`、`mail send result: status=sent`、ERROR 0。
