...
| **YouTube source articleize fix** | `383-INGEST-youtube-source-articleize-fix.md` | 344 の YouTube 記事が出ない件。root cause は YouTube source が `media_quote_only` のため `media_quote_pool` 追加後に articleize path へ進まず、title filter / caption / `【YouTube】` prefix に到達していなかったこと。repo fix + deploy 済み。Cloud Build `7ed5103a` SUCCESS、fetcher rev `yoshilover-fetcher-00436-7zq` 100%、`/health` OK、新 revision ERROR 0。次は自然 fire で YouTube path log / draft evidence 待ち。Scheduler / env / Secret / WP既存記事 / X / frontend は未変更。GitHub Issue #58。 |
