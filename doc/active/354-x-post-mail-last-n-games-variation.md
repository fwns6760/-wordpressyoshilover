...
## 11. 357 follow-up: 人間向け period 表示 / 全期間除外

2026-05-16 user 追加指示:

- 日付だけの `5/11〜5/16` は分かりにくい
- X 候補本文では `直近5試合` / `直近10試合` を前面に出す
- `7月成績` のような月別は分かりやすい
- 大手が出す全期間 / 今シーズン系はヨシラバー system では不要

357 での修正方針:

- `（5/11〜5/16・規定打席 5+）` ではなく `（直近5試合・規定打席5以上）`
- `（7/1〜7/31・規定打席 30+）` ではなく `（7月成績・規定打席30以上）`
- 守備位置別 OPS は `今シーズン` ではなく `直近7日`
- 月別は毎日ではなく月初 3 日だけ前月成績を巨人内 ranking として候補化
- X / SNS live post、Scheduler、env、Secret は触らない
