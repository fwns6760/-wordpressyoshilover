...
| `doc/active/380-x-post-mail-player-diversity-cap.md` (GH #54) | READY | 2026-05-18 07:00 JST の X 投稿候補 mail 8 件で `マルティネス` 3 件 / `岸田 行倫` 2 件を Gmail message id `19e37f4bb23efcbb` で確認。07:00 mail log は GCS DB `latest_game_date=2026-05-17 staleness_days=1 max=2`、production GCS object は `2026-05-18T01:01:30Z` 更新。repo-local DB は stale (`latest_game_date=2026-05-13`) だが production source of truth ではない。次 action は data first の same-player cap + next Giants row fallback。data不足時は source証拠付き news/opinion fallback を許可し、source に無い quote / 数字 / 断定は作らない。Scheduler / env / Secret / WP / X / SNS / production DB は不可触。 |
