...
| `doc/active/379-mail-publish-x-intent-button.md` (GH #53) | LIVE_DEPLOYED_OBSERVE | mail 内「公開してX投稿画面へ」ボタン実装 + deploy。 commit `9fdbeca`、 `src/publish_button_token.py` (HMAC + 24h expiry、 28 tests) + `src/publish_button_handler.py` (GET confirmation page + POST publish→302 X intent、 23 tests) + `src/server.py` GET/POST `/publish-and-tweet` + `src/publish_notice_email_sender.py` `publish_button_url` field 描画 + `src/publish_notice_scanner.py` `_resolve_fetcher_base_url` + populate。 Cloud Build `717d9e6e` (fetcher 2m7s) + `ea417159` (publish-notice 4m3s) SUCCESS、 両 image `379-publish-button-9fdbeca`。 deploy: fetcher rev `00425-vmd` 100%、 publish-notice Job Ready=True。 smoke curl: `/health` 200 / `/publish-and-tweet?post_id=abc` 400 / `?token=invalid` 403 全て想定通り。 X API / AUTO_TWEET / SNS 自動投稿は一切 touch なし (X intent URL 経由で user 手動投稿のみ)。 publish_notice + publish_button 系 396 tests passed、 regression 0。 env / Secret / Scheduler 未変更 (FETCHER_PUBLIC_BASE_URL / PUBLISH_BUTTON_TOKEN_SECRET 共に default fallback で動作)。 mail 自然 fire 後の user 受信目視 verify 待ち。 |
