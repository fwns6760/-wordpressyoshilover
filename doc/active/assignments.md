...
| `doc/waiting/382-MKT-brand-radar-x-search-fresh-news.md` (GH #56) | BLOCKED_USER | user 要望「巨人だから新鮮なニュースが良い」「ヨシラバーのポストを見たいと思わせる」。調査で local `hermes` command は未導入、ただし GCP Secret `yoshilover-grok-api-key` は存在し、`yoshilover-fetcher` には `GROK_API_KEY` が wired 済み。repo には xAI Responses API `x_search` 既存実装あり。PC off 前提なら Hermes local 常駐ではなく GCP direct xAI API key + `brand-radar` Job が本線。user 判断待ち: A) GCP direct xAI API key、B) local Hermes smoke first、C) article-only radar、daily x_search cap、new Job vs x-post-mail replace。Scheduler / env / Secret / WP / X は未変更。 |
