...
| `doc/active/379-mail-publish-x-intent-button.md` (GH #53) | LIVE_SPEC_VERIFIED | v1 `9fdbeca` (text URL + endpoint + token) → v2 `38bfedc` (HTML button + GCS one-shot) → evidence doc `ee508e9`。 v2 build: `f0c171e7` (fetcher 1m25s) + `1c353967` (publish-notice 3m37s) SUCCESS、 image `379-button-oneshot-38bfedc`。 deploy: fetcher rev `00426-45l` 100% + publish-notice Job Ready=True。 **End-to-end LIVE verify 実行 (PM)**: publish-notice JOB 手動 execute `publish-notice-l99t5` SUCCESS、 real draft `68962` で POST → HTTP 302 + Location `x.com/intent/tweet?text=...&url=https%3A%2F%2Fyoshilover.com%2F68962` + WP REST status=`publish` 確認。 2nd POST = 302 idempotent + GCS `is_consumed=True` (one-shot 効果)、 GET 同 token = 200「既に公開済」 page (status 検出正)。 spec 9 項目 (1-click publish+X intent / user 手動 / token 期限 / 1 回限り / 二重 click / draft only / publish 不変 / X API 無 / confirmation page) 全 OK evidence 残。 副作用: post 68962 が test publish 化済 (内容問題あれば WP admin or mail 非公開 button)。 422 tests pass、 regression 0。 env / Secret / Scheduler 未変更。 |
| `doc/active/380-x-post-mail-player-diversity-cap.md` (GH #54) | LIVE_DEPLOYED_OBSERVE | 2026-05-18 07:00 JST mail は `マルティネス` 3 件 / `岸田 行倫` 2 件、12:01 JST mail も旧 image `376-lineup-spread-e488052` で浦田俊輔 3 件 / マルティネス 3 件を Gmail id `19e3907b7bdba8e5` で確認。原因は repo 実装 `31aa4e7` 未 deploy。Cloud Build `ba632b1d-0bdd-4a09-b314-e7b8b3ea1c3e` SUCCESS、image `x-post-mail-lane:380-player-diversity-31aa4e7` digest `sha256:64e78e6f799...`、Cloud Run Job generation `20` に image only deploy。13:07 手動 execution `x-post-mail-lane-d6w4h` SUCCESS、Gmail id `19e39446c4ac0f79` は候補10件すべて focus player 別、同一 focus repeat 0、mixed label `巨人Xポスト案`。py_compile PASS、pytest/unittest `tests.test_x_post_mail` 89 passed。Scheduler / env / Secret / WP / X / SNS / production DB は未変更。dry-run execution `x-post-mail-lane-bjs5v` は `--args` が CMD を置換して起動失敗、追加 mail なし。次回自然 fire でも観察。 |
