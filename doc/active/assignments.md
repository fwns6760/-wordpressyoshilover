...
| `369-QA-short-player-event-title-quality` | IN_FLIGHT | GitHub Issue #38。68539 / 68608 型。`浦田俊輔、発言` / `浦田俊輔、安打` のような短すぎる `選手名 + 薄いイベント語` title を、同じX source本文内の literal な `2回適時打` / `3安打1打点猛打賞` で補強する。既存 335 は短すぎ引用、290 は弱title rescue親で、今回のplayer event fixtureは未カバー。記憶再構成 / silent skip / 自己評価OKは禁止。Scheduler / env / Secret / X / SNS / mail 条件は変更しない |
