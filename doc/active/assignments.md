...
| `370-QA-staff-x-web-dedupe` | IN_FLIGHT | GitHub Issue #39。68649 / 68653 型。368 の `player_name` 前提から漏れた杉内投手コーチなどの staff/manager/coach quote X+Web 重複を、staff subject + quote/event token で同 fire 消費する。WP既存 post は 68649 publish を X embed + source excerpt のまま title/headline 補正、68653 duplicate は trash 済み。記憶再構成 / silent skip / 自己評価OKは禁止。Scheduler / env / Secret / X / SNS / mail 条件は変更しない |
