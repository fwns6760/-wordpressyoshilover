...
| `doc/active/386-INSIGHT-no-game-day-normal-exit.md` (GH #61) | REVIEW_NEEDED | 2026-05-18(月) 試合なしで `insight-nightly` が `auto_resolve_all_failed` / exit 2 になった件。NPB日程HTMLが読めて parse できるが対象日 slug 0 件なら `status=no_game_day` / exit 0 に分岐し、日程取得失敗は従来通り blocked / exit 2 のままにする。tests: insight nightly/schedule/logging 33 OK、compileall / py_compile / AST / diff-check OK。deploy pending。Scheduler / env / Secret / WP既存記事 / X / frontend は不可触。 |
