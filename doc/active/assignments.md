...
| `376-QA-person-tag-routing-and-noindex` | REVIEW_NEEDED | GitHub Issue #49。選手・首脳陣・OBの人物タグを事前作成し、RSS記事作成時は既存 WP tag ID にだけ自動付与する。2選手記事は2タグ、runtime tag新規作成なし、missing / ambiguous / no hit は log 化。tag archive は `noindex, follow`。py_compile PASS、person tag / WP tag / noindex / RSS integration 16 passed、WP client 66 passed、RSS related 41 passed / 3 xfailed。WP tag sync / Cloud Run deploy / Scheduler / env / Secret / X / SNS は未実行 |
