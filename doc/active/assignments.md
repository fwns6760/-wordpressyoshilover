...
| `doc/active/382-MKT-brand-radar-x-search-fresh-news.md` (GH #56) | REVIEW_NEEDED | user 要望「巨人だから新鮮なニュースが良い」「ヨシラバーのポストを見たいと思わせる」。repo implementation landed: `src/brand_radar.py` + `src/tools/run_brand_radar_mail.py` + `tests/test_brand_radar.py`。3候補の `ヨシラバー投稿企画案｜巨人ニュース鮮度レーダー` mail、記事ソース優先、X Search evidence / unavailable明示、HTTP 401/403 は `x_search_auth_required`、default dry-run、no X live post、no WP mutation。tests: py_compile PASS、unittest 13 OK。network-approved source dry-run: candidates_sent=1、x_search_calls_used=0、DeNA-looking general RSS は `non_giants_topic` で reject、Full-Count 巨人 坂本候補のみ残存。Live GCP deploy / `GROK_API_KEY` runtime wiring / Scheduler / cadence / daily cap は未実施。 |
| `doc/waiting/383-QA-fan-voice-x-search-precision.md` (GH #57) | BLOCKED_USER | user 要望「本文の皆の声も同じ仕様で X Search で精度を上げたい」。現行 `💬 ファンの声（Xより）` は `fetch_fan_reactions_from_yahoo()` の Yahoo realtime path が主経路。382 と同じく GCP direct xAI API + `x_search` を本線、Hermes Agent は local/manual smoke または fallback。X Search URL-backed fan voice を優先し、empty/error は visible log + Yahoo fallback、subject overlap / freshness / low-value rejection / cap を ticket acceptance に固定。ticket-only request のため実装は user GO 待ち。Scheduler / env / Secret / WP / X は未変更。 |
