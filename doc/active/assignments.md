...
| `doc/active/382-MKT-brand-radar-x-search-fresh-news.md` (GH #56) | REVIEW_NEEDED | user 要望「巨人だから新鮮なニュースが良い」「ヨシラバーのポストを見たいと思わせる」。repo implementation landed: `src/brand_radar.py` + `src/tools/run_brand_radar_mail.py` + `tests/test_brand_radar.py`。3候補の `ヨシラバー投稿企画案｜巨人ニュース鮮度レーダー` mail、記事ソース優先、X Search evidence / unavailable明示、HTTP 401/403 は `x_search_auth_required`、credential は `XAI_OAUTH_BEARER_TOKEN` / `XAI_BEARER_TOKEN` / `XAI_API_KEY` 優先で legacy `GROK_API_KEY` fallback、default dry-run、no X live post、no WP mutation。paid xAI API は default disabled (`x_search_cap=0`)。Hermes docs evidence により `@yoshilover6760` Premium + Hermes OAuth は GCP smoke 実験として追う。tests: py_compile PASS、unittest 17 OK。network-approved source dry-run: candidates_sent=1、x_search_calls_used=0、`x_search_disabled_no_paid_api` 明示、DeNA-looking general RSS は `non_giants_topic` で reject、Full-Count 巨人 坂本候補のみ残存。Live GCP deploy / Hermes OAuth smoke / Secret / Scheduler / cadence は未実施。 |
| `doc/waiting/383-QA-fan-voice-x-search-precision.md` (GH #57) | BLOCKED_USER | user 要望「本文の皆の声も同じ仕様で X Search で精度を上げたい」。現行 `💬 ファンの声（Xより）` は `fetch_fan_reactions_from_yahoo()` の Yahoo realtime path が主経路。382 と同じく paid xAI API は default disabled、`@yoshilover6760` Premium + Hermes OAuth の GCP smoke 成功後に X Search URL-backed fan voice を優先候補にする。empty/error は visible log + Yahoo fallback、subject overlap / freshness / low-value rejection / cap を ticket acceptance に固定。ticket-only request のため実装は user GO 待ち。Scheduler / env / Secret / WP / X は未変更。 |
