...
| `371-QA-disable-game-live-source-policy` | IN_FLIGHT | GitHub Issue #40。user 方針「絞らないでいい。重複だけがいや」。試合あり 17:00-21:30 JST の `game_live_source_policy` をデフォルト解除し、サンスポX/Webなど通常sourceも流す。重複は既存 same-fire / cross-family / X+Web dedupe gates で止める。Scheduler / env / Secret / X / SNS / mail 条件は変更しない |
