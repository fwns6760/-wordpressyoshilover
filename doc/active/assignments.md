...
| `doc/active/380-x-post-mail-player-diversity-cap.md` (GH #54) | IN_FLIGHT | 2026-05-18 07:00 JST mail は `マルティネス` 3 件 / `岸田 行倫` 2 件、12:01 JST mail も旧 image `376-lineup-spread-e488052` で浦田俊輔 3 件 / マルティネス 3 件を Gmail id `19e3907b7bdba8e5` で確認。13:07 手動 execution `x-post-mail-lane-d6w4h` / Gmail id `19e39446c4ac0f79` でも浦田俊輔が再登場したため、user-visible acceptance は未達。follow-up は GCS dedup JSONL に `focus_player / metric / period_label` を記録し、直近24h player history から既出 player を避ける。ranking 内に別の巨人 player があれば差し替え、無ければ `player_history_skip` を log して news/opinion fallback で別 player を補充する。旧 `signature` only record 互換は維持。Scheduler / env / Secret / WP / X / SNS / production DB は変更しない。 |
