...
| **Lane QQ / body template v2 section header rename + H3 reduction** | REPO_IMPL_READY | `ENABLE_BODY_TEMPLATE_V2` を default OFF で追加し、social / manager / game / farm / notice / recovery / generic の section header rename と H3<=2 render を preview-only で実装。audit doc と preview doc を追加し、full pytest baseline を確認中。次は Claude が preview doc review / commit review / user preview judgment を進める |
