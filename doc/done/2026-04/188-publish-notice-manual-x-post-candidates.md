...
- [x] `build_body_text()` が既存 5 行情報に加えて `manual_x_post_candidates` block を出す。
- [x] 3 候補すべてが 280 字以内。
- [x] blank summary でも `(なし)` summary 行を維持し、候補生成で落ちない。
- [x] `send()` の real path が新しい body を bridge に渡す。
