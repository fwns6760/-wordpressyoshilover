...
- 2026-05-20 401 runtime refresh: PASS。`1fd6a6c` で source body excerpt を `600` -> `1200` にし、同時に `youtube_review_notice` caption を quote-only にした。`src/tools/manual_intake.py` は `SOURCE_BODY_EXCERPT_MAX_CHARS = 1200`、`src/rss_fetcher.py` は X outbound / auto RSS excerpt 呼び出しを `max_chars=1200` に統一。
- 2026-05-20 pre-deploy baseline refresh: PASS。`git diff --check` OK、`python3 -m py_compile src/rss_fetcher.py src/tools/manual_intake.py tests/test_rss_fetcher_youtube_caption_section.py tests/test_rss_fetcher_youtube_integration.py` OK、`python3 -m compileall -q src tests` OK、AST parse `AST OK 618`、sandbox外 `python3 -m pytest` は `5398 passed, 1 xfailed, 3 xpassed, 4 warnings`。sandbox内 full pytest は localhost socket 制限起点で 11 failed だったため、同一 baseline を通常権限で再実行して pass を確認。
- 2026-05-20 deploy / log 数値 diff refresh: PASS。manual-intake-service は revision `manual-intake-service-00088-vn8` / image `manual-intake-service:401-excerpt-1200-1fd6a6c` / generation `101/101` / traffic latest 100% / `/health` `{"ok": true}` / 新 revision ERROR log 0。fetcher は clean archive `d1c5d7a` から Cloud Build `f9799b40-89b7-400d-9b21-2ba972e80189` SUCCESS、image `yoshilover-fetcher:401-excerpt-quote-d1c5d7a` digest `sha256:8dcae877...`、revision `yoshilover-fetcher-00457-pcr` 100%、generation / observedGeneration `597/597` -> `599/599`、`/health` OK、新 revision ERROR log 0。env / Secret / Scheduler / RUN_DRAFT_ONLY / WP既存記事 / X / manual `/run` fire は未変更。
- 2026-05-20 final current-state refresh: PASS。後続の 402 deploy で current は fetcher `yoshilover-fetcher-00458-2dq` / image `402-meta-fallback-671df22` / generation `600/600`、manual-intake-service `manual-intake-service-00089-xtf` / image `402-meta-fallback-671df22` / generation `102/102` に進行。`671df22` は `1fd6a6c` / `d1c5d7a` descendant のため、1200 字化と quote-only guard は live 維持。両 `/health` OK、新 revision ERROR log 0。
