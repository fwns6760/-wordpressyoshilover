...
## T-028 🟡 `yoshi-front-card` の `.is-yoshi-front-density` scope が配線されていない

**発見日**: 2026-04-24
**発見者**: Claude Code（監査役、front_refresh 便 read-only 確認）
**影響**: `custom.css` に `.is-yoshi-front-density .p-postList__body` スタイルと `.yoshi-front-card__badge/chip/summary/detail` 群が定義されているが、body / wrapper にそのクラスが付かず、スタイルがどこにも効いていない

**実測**:
- `https://yoshilover.com/` body class: `home blog wp-theme-swell wp-child-theme-swell_child -index-off -sidebar-on -frame-on -frame-on-sidebar` → `is-yoshi-front-density` 無し
- `yoshi-front-card` クラスも DOM 0 件

**次アクション**:
- plugin 側で `body_class` filter に `is-yoshi-front-density` を足すか、もしくは SWELL 記事一覧 template の `.p-postList` に class を噛ませる仕組みを追加
- 本件は front_refresh 便スコープ外（別便判断）
