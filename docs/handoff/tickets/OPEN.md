...
## T-022 🟠 18 時付近の `/run` 発火が弱い疑い（調査完了・土曜 gap 修正待ち）

**発見日**: 2026-04-18 夜
**発見者**: よしひろさん（「18時発火よわい。昨日もだが」）
**影響**: 夕方の RSS 取り込みが薄い → 記事が時刻帯で偏る可能性

**第22便 調査結果**: `docs/handoff/codex_responses/2026-04-18_22.md`

**結論**:
- **2026-04-18(土) 18:00-19:59 JST** は scheduler gap が実在
  - `giants-weekend-post`: `0,30 16,17 * * 0,6`
  - `giants-weekend-eve`: `0,30 20-22 * * 0,6`
  - したがって **18:00 / 18:30 / 19:00 / 19:30 JST が空白**
- **2026-04-16(木) / 2026-04-17(金)** は `/run` request と flow summary が正常に存在
  - 18-20 時帯の薄さは未発火ではなく、`pregame_started` / `live_update_disabled` / `sns_polluted` / `social_too_weak` / `not_giants_related` 側
- `error_count=0`、HTTP 200 のため **処理落ち / fetch failure 主因ではない**

**状態**: **部分 RESOLVED**

- 土曜 gap の原因切り分けは完了
- 修正は第23便で実施予定
- 平日側の filter/policy 問題は **T-023** として分離

**次アクション**:
1. 第23便で weekend scheduler gap を修正
2. 自然発火で 18:00-19:30 JST が埋まることを確認
3. 確認完了後、T-022 を RESOLVED へ移動

**Codex向け指示書**:
- 調査便: `docs/handoff/codex_requests/2026-04-18_22.md`
- 修正便: `docs/handoff/codex_requests/2026-04-18_23.md`

---

## T-023 🟡 平日18-20時の `/run` は発火しているが filter/policy で空振りしやすい

**発見日**: 2026-04-18 夜
**原因切り分け日**: 2026-04-18（第22便）
**発見者**: よしひろさん / Codex 調査
**影響**: 平日夕方でも候補が残りにくく、記事生成数が不安定

**第22便の事実**:
- 2026-04-16(木) / 2026-04-17(金) の 18:00-20:00 JST は `/run` request が正常発火、HTTP 200、`error_count=0`
- ただし flow summary 上は以下で空振りしやすい
  - `pregame_started`
  - `live_update_disabled`
  - `sns_polluted`
  - `social_too_weak`
  - `not_giants_related`
  - 一部 `history_duplicate`
- 2026-04-17(金) 18:00 / 19:30 / 20:00 JST は `pregame` 候補が残っても `pregame_started` で 0 件化
- 2026-04-16(木) 19:00-20:00 JST は `prepared_category_counts={}` で、候補段階から filter に落ちている

**判定**:
- インフラ障害ではない
- scheduler 追加だけでは解消しない
- 「本当に捨てるべきか」の運用判断を含むため、土曜 gap 修正とは分離する

**検討論点**:
1. `pregame_started` の発動時刻が厳しすぎないか
2. `live_update_disabled` を平日夕方にどう扱うか
3. social 系の `sns_polluted` / `social_too_weak` の閾値が強すぎないか
4. 18-20 時帯だけ運用 policy を緩める価値があるか

**次便候補**:
- 7d 窓で 18-20 時帯の skip_reason 分布を追加集計
- `pregame_started` 発火条件と実際の試合開始時刻の突合
- social 系 filter の誤検知率確認
