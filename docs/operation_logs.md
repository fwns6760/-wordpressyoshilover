...
| `media_xpost_embedded` | social_news 記事の source tweet を本文冒頭に埋め込んだ post 作成後 | `post_id`, `title`, `source_type`, `media_handle`, `media_url`, `position` | `src/rss_fetcher.py:7741` | source tweet 埋め込みが本番記事に載ったか確認 | `social_body_template_applied` はあるのに出ない、`media_handle` が空で続く |
