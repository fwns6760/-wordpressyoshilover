...
| `article_ai_route_override` | 通常の AI 経路から notice/farm のためルートを上書き | `original_category`, `effective_category`, `article_subtype`, `effective_ai_mode`, `title` | `src/rss_fetcher.py:3863` | safe fallback 対策がどこで効いているか確認 | `effective_category` が偏る、または特定 subtype に集中 |
...
### 11. safe fallback と guardrail を見る
