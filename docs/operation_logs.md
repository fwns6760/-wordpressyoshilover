...
| `media_xpost_embedded` | social_news 記事の source tweet、または notice / manager 記事に紐付いた公式・報道ポストを本文冒頭に埋め込んだ post 作成後 | `post_id`, `title`, `source_type`, `media_handle`, `quote_account`, `media_url`, `embed_section_type`, `match_reason`, `match_score`, `position` | `src/rss_fetcher.py:7757` | source tweet / 公示ポスト / 報道ポスト埋め込みが本番記事に載ったか確認 | `social_body_template_applied` や `notice_body_template_applied` / `manager_body_template_applied` はあるのに出ない、`embed_section_type` や `match_reason` が想定外、`media_handle` が空で続く |
