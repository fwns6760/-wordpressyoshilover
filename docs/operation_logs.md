...
| `media_xpost_embedded` | social_news 記事の source tweet、または notice 記事に紐付いた公式公示ポストを本文冒頭に埋め込んだ post 作成後 | `post_id`, `title`, `source_type`, `media_handle`, `media_url`, `match_reason`, `match_score`, `position` | `src/rss_fetcher.py:7757` | source tweet / 公示ポスト埋め込みが本番記事に載ったか確認 | `social_body_template_applied` や `notice_body_template_applied` はあるのに出ない、`match_reason` が想定外、`media_handle` が空で続く |
