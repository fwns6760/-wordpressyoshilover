...
| `rss_fetcher_flow_summary` | 1 run 終了時 | `skip_reasons`, `prepared_category_counts`, `prepared_subtype_counts`, `created_category_counts`, `created_subtype_counts`, `publish_skip_reason_counts`, `x_skip_reason_counts`, `x_ai_generation_count`, `x_ai_generation_limit` | `src/rss_fetcher.py:6064` | どこで落ちているか、どのカテゴリが作られたかを見る | `social_too_weak` や `comment_required` が急増、`featured_media_missing` が急増、`x_ai_generation_count` が上限張り付き。`x_skip_reason_counts.live_update_x_post_disabled` は synthetic live update 抑止時の想定値 |
...
| `[X投稿スキップ] post_id=... reason=...` | X投稿しない条件に該当 | `src/rss_fetcher.py:6024` | X 側の skip 理由確認 | `daily_limit` や `missing_featured_media` が急増。`live_update_x_post_disabled` は `ENABLE_X_POST_FOR_LIVE_UPDATE=0` で synthetic live update を意図的に除外した状態 |
