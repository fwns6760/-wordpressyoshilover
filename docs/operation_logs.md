...
| `fact_check_notify_started` | `/fact_check_notify` または `fact_check_notifier --send` が fact check メール処理を開始した時 | `since`, `checked_posts`, `red`, `yellow`, `green`, `send` | `src/fact_check_notifier.py` | 毎朝の acceptance メールが実際に走ったか確認 | Scheduler 時刻に出ない、`checked_posts=0` が不自然に続く |
| `fact_check_email_sent` | Gmail SMTP 送信が成功した時 | `since`, `checked_posts`, `red`, `yellow`, `green`, `to_email`, `subject` | `src/fact_check_notifier.py` | 毎朝メールが送れたか、件名と件数が妥当か確認 | Scheduler 実行はあるのに本イベントがない |
| `fact_check_email_failed` | Gmail SMTP 送信か Secret Manager 取得に失敗した時 | `since`, `checked_posts`, `red`, `yellow`, `green`, `error_type`, `error`, `to_email` | `src/fact_check_notifier.py` | app password / SMTP / Secret IAM の障害切り分け | 1件でも要確認。`secret access failed` や `SMTPAuthenticationError` は即対応 |
| `fact_check_email_demo` | Gmail app password 未設定のためデモモードで本文を stdout に出した時 | `subject`, `to_email`, `from_email`, `text_preview` | `src/fact_check_notifier.py` | 朝までに SMTP 設定が未完了でも本文プレビューを確認 | 本物送信のつもりなのに出続ける |
...
- 将来 Cloud Logging の `jsonPayload` に正規化した場合は、各イベントとも `jsonPayload.event="..."` に置き換えれば同じ意味で使える。

## 主要イベントのクエリ集

今日の受け入れ試験で主に見るイベントだけ先にまとめる。各イベントとも、先頭のクエリが現行で使うもの、2本目が `jsonPayload.event` へ寄せた等価クエリ。

### `run_started`

何が見えるか:

- `/run` 開始時点の `run_draft_only` / `auto_tweet_enabled` / `publish_require_image` / `revision`
...
textPayload:"run_started"
```

```text
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
jsonPayload.event="run_started"
```
...
- 朝の acceptance fact check がいつ走ったか
- `since=yesterday` で何件見に行ったか

いつ見るか:

- 毎朝 7:00 の scheduler 実行後
- 手動 dry run の直後

```text
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
textPayload:"fact_check_notify_started"
```

```text
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
jsonPayload.event="fact_check_notify_started"
```
...
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
textPayload:"fact_check_email_sent"
```

```text
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
jsonPayload.event="fact_check_email_sent"
```
...
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
textPayload:"fact_check_email_failed"
```

```text
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
jsonPayload.event="fact_check_email_failed"
```
...
- app password 未設定でも、送るはずの本文の先頭プレビューが見える

いつ見るか:

- 今夜の実装確認
- 明朝の app password 設定前

```text
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
textPayload:"fact_check_email_demo"
```
...
- デモモードで fact check 集計までは成功したこと

いつ見るか:

- 今夜の実装確認
- SMTP 設定前の scheduler dry run

```text
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
textPayload:"fact_check_email_demo_ready"
```
...
- draft-only 受け入れ試験中でもアイキャッチ欠落がある post を拾える
...
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
jsonPayload.event="featured_media_observation_missing"
```
...
- `live_update` が publish gate で意図どおり隔離されているか
...
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
jsonPayload.event="live_update_publish_disabled"
```
...
- subtype 別 publish gate に止められた記事
...
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
jsonPayload.event="publish_disabled_for_subtype"
```
...
- `rss_fetcher_flow_summary.x_skip_reason_counts` の中で subtype 別 X gate が効いた run
...
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
jsonPayload.event="rss_fetcher_flow_summary"
jsonPayload.x_skip_reason_counts.x_post_disabled_for_subtype:*
```
...
- subtype 別 X gate に止められた post 単位の明細
...
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
jsonPayload.event="x_post_subtype_skipped"
```
...
- B.5 selector が `own_source / npb_notice / manager_media / none` のどれで評価されたか
...
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
jsonPayload.event="media_xpost_evaluated"
```
...
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
jsonPayload.event="media_xpost_skipped"
```
...
- B.5 で本文に実際に埋め込まれた source tweet / 報道ポスト
...
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
jsonPayload.event="media_xpost_embedded"
```
...
- Gemini preview が生成された post と `fallback_used`
...
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
jsonPayload.event="x_post_ai_generated"
```
...
- Gemini preview の失敗種別と fallback 発生
...
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
jsonPayload.event="x_post_ai_failed"
```
...
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
jsonPayload.event="article_image_refetched"
```
...
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
textPayload:"featured_image_fallback_applied"
```

```text
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
jsonPayload.event="featured_image_fallback_applied"
```
...
- 既存 post / draft から featured media を再利用した救済ケース
...
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
jsonPayload.event="featured_media_reused_from_existing_post"
```

### `rss_fetcher_run_summary` / `rss_fetcher_flow_summary`

何が見えるか:

- run 単位の KPI、draft 作成数、カテゴリ分布、publish/X gate の詰まり
...
resource.type="cloud_run_revision"
resource.labels.service_name="yoshilover-fetcher"
(jsonPayload.event="rss_fetcher_run_summary" OR jsonPayload.event="rss_fetcher_flow_summary")
```
...
resource.labels.service_name="yoshilover-fetcher"
textPayload:"rss_fetcher_flow_summary"
textPayload:"created_category_counts"
```
