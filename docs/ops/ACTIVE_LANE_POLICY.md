...
- 原則として、現場ClaudeはACTIVE 2件を埋める。1件だけで回すのは例外。
- ACTIVE 1件運用が許されるのは、次のいずれかを満たす場合だけ。
  - 2件目候補が同一ファイル / 同一deploy対象 / 同一判断境界で衝突する。
  - 2件目候補がUSER_DECISION_REQUIREDだけで、Acceptance Pack未完成。
  - 2件目候補がGemini増 / mail増 / env/flag / Scheduler / SEO / source追加 / cleanup mutationを伴う。
  - 既存ticket内のread-only / doc-only / test plan / Pack補強 / rollback planが本当に残っていない。
- ACTIVE 1件運用にする場合、現場Claudeは「なぜ2件目を昇格しないか」を短く正本に残す。userに選ばせない。
...
- ACTIVEが1件に減ったら、現場ClaudeはREADY_NEXTまたは既存ticket内subtaskから2件目を自動補充する。
