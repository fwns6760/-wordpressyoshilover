...
| BUG-004 | メールに出ないskip、候補が消える事故源。publish=0原因分解とnarrow publish-path unlockも含める | userから候補が見えない。公開ゼロが続く。受け入れ条件違反 | デグレ防止 / observe / publish path品質改善 | 277 / 289 / 290 / 291 / 292 / 293 / 295 / 288 Phase 1/2 | 既存ticketに吸収 | Claude / Codex | silent skip pathごとにmail/ledger/log/user-visible経路を確認。publish=0原因をread-onlyで分解し、高信頼候補だけpublish pathへ戻す設計を作る | publish/review/hold/skip/errorのどれかで見える。高信頼条件(YOSHILOVER対象、source_url、subtype high confidence、numeric/body pass、placeholderなし、title明確、review/hold理由なし)を満たす候補だけpublish候補へ戻せる | Excelメモ行:「メールに出ない skip(候補が消える事故源)を deploy 前 grep」。追加方針: publish gate全体緩和ではなくnarrow unlock |
