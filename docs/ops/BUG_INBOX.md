...
- 2026-05-03時点のACTIVEは `BUG-004+291 publish=0原因分解 + narrow publish-path unlock` の1件。
- `292 body_contract_fail durable ledger方針` は独立ACTIVEにせず、BUG-004+291の解除条件・必須サブタスクとして扱う。
- `282-COST` と `298-Phase3` はOBSERVE、`245` はUSER_DECISION_REQUIRED。
...
- チケット番号の破壊的な振り直しは禁止。
- Excelだけを正本にしない。
- Excelはuser入力口ではなく、Codex/Claudeが整理結果を共有する管理ファイル。
...
- `docs/ops/AI_TEAM_OPERATION_MODEL.md`
...
| BUG-004 | メールに出ないskip、候補が消える事故源。publish=0原因分解とnarrow publish-path unlockも含める。292 body_contract_fail durable ledger方針とsubtask-7 guarded-publish gate reason auditも解除条件として吸収 | userから候補が見えない。公開ゼロが続く。受け入れ条件違反 | デグレ防止 / observe / publish path品質改善 | 277 / 289 / 290 / 291 / 292 / 293 / 295 / 288 Phase 1/2 / guarded-publish gate | 既存ticketに吸収 | Claude / Codex | silent skip pathごとにmail/ledger/log/user-visible経路を確認。publish=0原因をread-onlyで分解し、高信頼候補だけpublish pathへ戻す設計を作る。body_contract_failは通常メールではなくdurable ledger/logで後追いできる設計にする。narrow unlock後もpublish=0なら、draft化された候補がguarded-publish後段でpublish化されない理由をread-only特定する | publish/review/hold/skip/errorのどれかで見える。高信頼条件(YOSHILOVER対象、source_url、subtype high confidence、numeric/body pass、placeholderなし、title明確、review/hold理由なし)を満たす候補だけpublish候補へ戻せる。body_contract_failが黙って消えず、通常メールを増やさず、件数と理由を追える。draft化された候補がguarded-publishで止まる場合は、numeric/placeholder/body_contract/duplicate/freshness/backlog_only等の理由がledger/logで特定できる | Excelメモ行:「メールに出ない skip(候補が消える事故源)を deploy 前 grep」。追加方針: publish gate全体緩和ではなくnarrow unlock。292は独立ACTIVEにしない。2026-05-03現場報告: publish-only filterとnarrow unlockは有効、次はguarded-publish後段gate調査 |
...
| BUG-008 | mail送信pathにLLM呼び出しが追加されていないか | 通知だけでGemini/LLM費用が増える危険 | コスト / 通知 | 229 / 293 / 279-QA / publish-notice no-LLM invariant候補 | DONE 2026-05-03 (CLEAN) | Claude / Codex | publish-notice / mail sender pathのLLM import/call grep | mail pathのGemini/LLM call 0証拠が残る → 達成 | Excelメモ行:「mail 送信 path に LLM 呼び出しが追加されてないか」。 audit結果: docs/ops/bug008_mail_llm_audit.md (commit fba14e3)、14 mail path 関数 audit、LLM keyword 検出 2 (provider label 文字列のみ、callable SDK import / call は 0)、P1 昇格不要、CLEAN 確定 |
...
## BUG-004+291 publish回収条件(2026-05-03 user-confirmed)

BUG-004+291では、publish=0を回収するために「全部止める」でも「publish gate全体緩和」でもなく、次の高信頼記事タイプだけをnarrowにpublish pathへ戻す。

救う対象:

- 試合結果記事: 当日 + 巨人対象 + 相手 + スコアが title / source / meta / body のどこかから取れるもの。
- 先発投手成績: sourceに成績がないだけでは落とさない。ただしsourceにない投球回 / 被安打 / 失点 / 勝敗は本文に書かない。
- 監督・コーチコメント: 一部引用でもsourceが明確なら救う。postgameへ雑分類しない。
- 選手コメント: 必須。選手名とコメント元が明確ならscoreなしでもplayer_commentとして救う。
- 二軍結果: 必須。farm_resultは一軍postgameと分け、二軍 / 相手 / スコアが取れるなら救う。
- 二軍スタメン: 必須。farm_lineupはfarm_resultと分け、スタメン表 / 打順 / 選手名が取れるなら救う。
- pregame / 予告先発: 必須。probable_starter / pregame / lineupは救う。ただし試合後の古いpregameは出さない。
- 昇格・降格・復帰・二軍落ち・若手記事: 必須。notice / roster_notice / injury_recovery_notice / farm_player_result等へ寄せる。
...
- titleが弱い記事はpublishしない。ただし中身が明確なcomment / farm / roster系はdeterministic rescue候補にできる。
- AIでタイトルを自由生成しない。source/metaから取れる選手名・記事タイプ・相手・スコアだけを使う。
- 報知 / 日刊 / スポニチ / 公式 / X由来などで同内容が複数来た場合、基本は1本だけにする。
- duplicate guardは緩めない。
- この条件はBUG-004+291のticket本文にも同期する。新規ticket化しない。
