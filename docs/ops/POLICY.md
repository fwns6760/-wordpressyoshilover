...
## 25. Codex worker pool 自律管理(永続、user 発見の lane idle 禁止)

Claude は Codex Lane A / B の **state 管理者 + 一次受け + 次作業 dispatcher**。lane idle を user が発見する状態は禁止。

### 25.1 lane state 5 値

| state | 意味 | Claude 対応 |
|---|---|---|
| **running** | Codex bg 実行中 | task notification 待ち、5-10 min idle で軽量 progress check(memory: feedback_codex_idle_check_policy.md)|
| **completed** | Codex 完了通知受領 | 一次受け(test/diff/risk/cost/rollback verify)→ GO/HOLD/REJECT 圧縮 → repo 正本記録 |
| **blocked** | dependency 待ち(他 lane / user GO / production observe / 24h 安定 等)| blocker 明示 + CURRENT_STATE に記録 + 解除 timing 明記 |
| **failed** | Codex preflight stop / pytest regression / commit 失敗 等 | 失敗理由記録、retry 判定(target 修正 / scope 縮小 / abort 判断)|
| **idle** | running / completed / blocked / failed どれでもない、次作業未投入 | **Claude が即次チケット投入**(消化順 §25.4)、idle 状態維持禁止 |

### 25.2 completed 後の一次受け 5 step

1. **diff 確認**:`git log <ref>..HEAD --stat` で変更 file + 行数 verify
2. **test 確認**:Codex 報告の pytest count + 必要なら Claude 自身で再実行(memory: feedback_accept_pytest_baseline_required.md)
3. **risk 評価**:scope 違反 / 不可触 file 触り / 想定外 path 検出
4. **cost 評価**:Gemini call delta / Cloud Build / GCS write 増加なし verify
5. **rollback 確認**:env 1 コマンド or image revert path 明示済み
- 5 step 全 pass → GO 判定 + commit/push
- 1 つでも fail → HOLD/REJECT + 報告 + retry or abort

### 25.3 user GO 必須(自律投入禁止)
...
### 25.4 自律投入可能な低リスク作業 + 消化順

**自律投入 OK(user GO 不要、Claude 判断で次 lane fire)**:
- read-only 解析 / 設計 doc 整備
- doc-only(Pack draft / test plan / rollback plan / impl 順序整理)
- evidence 追記(OPS_BOARD entry update / INCIDENT_LIBRARY post-mortem)
- 既存チケット subtask 整理(新規 ticket 乱立禁止、既存追記)
- Acceptance Pack 18 項目 起草(impl 着手しない)

**消化順(2026-05-01 user 固定)**:
1. 298-Phase3 mail storm 第二波防止
2. 293-COST preflight skip visible 化 / 282 前提整理
3. 299-QA flaky / transient 整理
4. 282-COST flag ON 判断 Pack
5. 290-QA weak title rescue deploy 判断 Pack
6. 300-COST source-side 再評価 cost 解析
7. 288-INGEST source 追加 判断 Pack
8. 278-280 品質改善

### 25.5 lane idle 検出時の自律 dispatch flow

```
1. lane completed 通知受領
2. 一次受け 5 step 実行(§25.2)
3. 5 step 全 pass → commit/push、5 step 一部 fail → HOLD/retry
4. lane idle 化検出
5. 消化順から次の低リスク ticket を選ぶ(§25.4)
6. user GO 必須が混入していないか check(§25.3 該当なら HOLD + Pack 起草で代替)
7. Codex prompt 起草 + bg fire
8. lane state running へ遷移、step 1 へ戻る
```

### 25.6 Decision Batch 報告(user 細切れ確認禁止)

- P0/P1 incident 検出 → 即時 1 行報告
- それ以外 → 2 lane completed まで silent、両 lane 完了で 1 画面 Decision Batch 報告
- 中間 milestone 短報告は user 視認時のみ(「進んでる?」「動いてる?」等)、平時は silent

### 25.7 Claude rate limit / 監視不能時の引継ぎ(永続)

Claude session が rate limit / context 圧縮 / WSL 再起動 / 別端末で起動 で監視不能になる risk に備え、**`docs/ops/CURRENT_STATE.md` の `## Codex Worker Pool State` section に常時更新**:

```markdown
## Codex Worker Pool State(最後の更新: <timestamp>)

### Lane A
- state: running | completed | blocked | failed | idle
- task_id: <bg id>
- ticket: <ticket id>
- prompt: <short scope summary>
- expected_completion: <timestamp>
- result_path: <log file path or completion notification>

### Lane B
(同上)

### 最後の Claude 指示
- <Claude が直前に投げた指示の 1 行 summary>

### 次に読むべき結果
- <pending 結果 file path or task notification>

### user 判断が必要か
- 0 件 / Decision Batch / Acceptance Pack 提示 中
```
