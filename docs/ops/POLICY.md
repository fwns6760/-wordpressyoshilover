...
1. `docs/ops/CURRENT_OPERATION_CONTRACT.md`
2. `docs/ops/POLICY.md`
3. `docs/ops/CURRENT_STATE.md`
4. `docs/ops/OPS_BOARD.yaml`
5. `docs/ops/NEXT_SESSION_RUNBOOK.md`
6. `docs/ops/INCIDENT_LIBRARY.md`
7. `docs/ops/ACCEPTANCE_PACK_TEMPLATE.md`
8. `docs/ops/CLAUDE_FIELD_RULES.md`
...
- User: client / orderer, not field manager. Decision owner for high-risk choices only.
...
- User is not Claude/Codex's boss, dispatcher, ticket manager, log reader, or normal operations reviewer.
- User's normal role is limited to initial requirements, acceptance criteria, final `OK / HOLD / REJECT`, and sharing anomalies or field logs.
...
- Claude owns ACTIVE max-2 management, READY_NEXT promotion, worker dispatch, normal log interpretation, and routine HOLD classification.
- Claude owns evidence management and rollback judgment before escalating to the user.
- User-facing output is limited to `USER_DECISION_REQUIRED`, P0/P1 abnormality, rollback, irreversible-change decision, important state arrival, and acceptance judgment.
- ChatGPT in the meeting room is an external objective consultant, not the owner of field decisions.
- Claude must not offload normal field judgment to ChatGPT. Claude keeps `CLAUDE_AUTO_GO` judgment, ACTIVE management, Codex dispatch, evidence management, and rollback judgment.
- Claude may escalate to ChatGPT only for GO/HOLD/REJECT framing, over-stop / over-go / mixed-scope check, policy conflict, irreversible change, or uncertainty about `USER_DECISION_REQUIRED`.

## 3. Production Change Classification

Current daily gate is `docs/ops/CURRENT_OPERATION_CONTRACT.md`.
Under the current contract, deploy / flag / env / Scheduler / SEO / source / Gemini / major mail / WP body / publish-state changes are `USER_DECISION_REQUIRED`.

### 3.1 CLAUDE_AUTO_GO

Claude may proceed without user GO only when all current-contract auto-go conditions are true:

- read-only
- doc-only
- preview-only
- deploy: none
- flag / env / Scheduler / SEO / source change: none
- Gemini call increase: none
- mail notification-condition change: none
- WP body change: none
- publish-state change: none
- rollback target is clear where applicable
- same deploy target has no conflict

Claude records evidence internally and reports only user-visible events defined in the current contract.
...
HOLD must not be a vague safety label. If Claude/Codex uses HOLD, it must name exactly one concrete blocker:

- rollback-impossible or rollback target missing
- verification-impossible or required evidence cannot be collected
- user authority required

If none of these applies and the `small GO` conditions in section 22 are satisfied, Claude must not keep the ticket in indefinite HOLD.

#### HOLD blocker format (2026-05-03 user lock)

`安全のため HOLD` `user 確認が必要` `正常系報告` `blocker なし停止` は禁止。HOLD する場合は具体 blocker 1 件を 5 項目 format で出す。

1. 何が危険か
2. どの evidence が不足か
3. Claude / Codex 側で確認可能か
4. 確認コマンド or 確認対象
5. 推奨判断: `GO` / `HOLD` / `REJECT`

HOLD ではなく前進判定にしてよい条件:

- scope が 1 ticket
- rollback target あり
- restore 手順あり
- デグレ試験あり
- post-deploy verify あり
- stop condition あり
- Gemini call / mail / SEO / source / Scheduler の増加や変更がない、または user GO 済み

迷った時の分類 (user に相談しない):

- `SELF_CHECK` — Claude 単独で repo / git / log を見て確認可能
- `CODEX_CHECK` — Codex 便で確認 (read-only / dry-run)
- `HOLD_REASON` — 上記 5 項目 blocker で具体化

Mindset: `最悪ケースから守る` ではなく `現状から改善する` 視点。臆病 stop 禁止、戻せる範囲は前進。
...
After any approved production reflection, Claude must collect post-deploy verify and production-safe regression evidence before the ticket can become `OBSERVED_OK` or `DONE` candidate.
...
日々の正本は `docs/ops/CURRENT_OPERATION_CONTRACT.md`。
現在の契約では deploy / flag / env / Scheduler / SEO / source / Gemini / mail 条件大改修 / WP本文 / publish状態変更は `USER_DECISION_REQUIRED`。

### 14.1 CLAUDE_AUTO_GO(本番反映なし、user 確認不要)

以下を満たす場合だけ、Claude/Codex側で細切れ確認せず前進する:

1. read-only / doc-only / preview-only
2. deployなし
3. flag / env / Scheduler / SEO / source変更なし
4. Gemini call増加なし
5. mail通知条件変更なし
6. WP本文変更なし
7. publish状態変更なし
...
- 現場監督ではない
- 上司ではなくクライアント / 発注者である
- 本来の役割は、初期要件定義、受け入れ条件の提示、最終的な `OK / HOLD / REJECT`、違和感や現場ログの共有だけ
- Claude の推奨判断に対して `OK / HOLD / REJECT` を返すだけ
- 高 risk choice の最終責任(POLICY §2 整合)
- 細かい技術 / 運用判断は委任済(本 §15)
- 次に何をやるかの選定、チケット整理、worker dispatch、通常ログ確認、正常系報告の読解、細かいGO/HOLD判断、Claude/Codex間の仲介、実装手順の管理はuserへ戻さない。
- ACTIVE管理、Codex dispatch、`CLAUDE_AUTO_GO`判定、HOLD理由整理、evidence管理、rollback判断はClaude側で持つ。
- 会議室ChatGPTは客観コンサルであり、現場判断の丸投げ先ではない。通常の進行判断はClaude側で処理する。

### 15.6 適用境界

| 状態 | Claude の動き | user 接点 |
|---|---|---|
| CLAUDE_AUTO_GO(§3.1)| read-only / doc-only / preview-only の前進 + evidence 記録 | 正常系報告不要 |
...
本日 Codex A round 28(デグレ + deploy gate 9 軸)+ Codex B round 17(コスト + storm 8 軸)audit 由来の追加 guard。以後は `CURRENT_OPERATION_CONTRACT.md` に従い、deploy は user 判断対象、検証・Pack化・UNKNOWN潰しは現場側で持つ。
...
## 21. Prompt-ID Cost Review Gate(永続、改修 #4)

§19.6 は `prompt_template_id` / cache 構造変更時の cost review 必須化を既に示している。本 §21 はそれを Pack 運用まで下ろし、ticket 単位ではなく **prompt-id 単位**で cost review を強制する恒久 gate である。

### 21.1 prompt-id definition

`prompt-id` は、prompt の差し替え・拡張・分岐追加・fallback/rescue 導入・cache key 変更の影響を安定して追跡するための識別子。

対象:

- `prompt_template_id`
- fixed-lane / rescue / fallback など prompt variant の安定 ID
- 明示 ID が未実装でも、prompt 動作を変える path は Pack 上で provisional ID を付ける(`path:function` 形式可)

1 ticket で複数 prompt-id を触る場合、**1 prompt-id = 1 review row** で分解する。ticket 合算だけで済ませない。

### 21.2 per-prompt-id required review fields

Acceptance Pack または design doc で、影響を受ける各 prompt-id について以下を必須記入:

- prompt-id
- activation path / touched path
- baseline Gemini calls/day
- Gemini delta upper bound/day
- prompt-derived mail path と mail volume estimate/hour, /day
...
`UNKNOWN`、`TBD`、placeholder、または ticket 合算だけで per-prompt 分解不能な記述は gate 未通過とみなす。

### 21.3 evaluation matrix(4 axes x 3 stages)

| stage | prompt-id attribution | Gemini delta | mail volume | cost upper bound |
|---|---|---|---|---|
| `PASS` | touched prompt-id が全列挙済み。未触達 path を含まない | baseline/day と delta upper bound/day が数値で固定。`0` または減少、または ticket 許容帯域内 | prompt-id 由来 mail path が列挙済み。増加なし、または MAIL_BUDGET 内で数値固定 | tokens/day・API calls/day・Cloud Run executions/day が数値で上限化済み |
| `REVIEW` | prompt-id は列挙済みだが、新規 prompt path / fallback path が追加される | 増加方向だが数値上限は確定済み。ticket の Acceptance Pack で user-facing review が必要 | 増加方向だが数値上限は確定済み。MAIL_BUDGET 内設計で stop condition も明記済み | cost 増加方向だが上限は確定済み。Pack 内で reason / rollback / observe plan が必要 |
| `STOP` | prompt-id 未記載、複数 prompt-id を合算だけで処理、または provisional ID すら置けない | baseline 不明、delta 不明、上限なし、または deploy 後しか見積もれない | mail path 未特定、volume 不明、または storm 再発上限が引けない | tokens/API/Cloud Run のいずれか不明、unbounded、または ticket-level summary と整合しない |

判定規則:

- 全 axis `PASS` のときのみ、通常の ticket classification へ進める
- `REVIEW` が 1 つでもあれば、Prompt-ID Cost Review field を含む Acceptance Pack 完成前に GO しない
- `STOP` が 1 つでもあれば `HOLD`。user へ投げず、Claude が UNKNOWN を潰す

### 21.4 stop conditions

以下のいずれかで prompt-id cost review gate は失敗:

- prompt-id を触る change なのに Prompt-ID Cost Review field が空
- per-prompt baseline / delta / mail volume / API estimate / cost upper bound のいずれか欠落
- prompt-id row の合計が ticket-level `Gemini / Cost Impact` または `Mail Volume Impact` と整合しない
- prompt-id 変更なのに rollout 後観測でしか上限が決まらない
- new prompt-derived mail path があるのに volume estimate が無い

この状態では `GO` 禁止。state は `HOLD`。

### 21.5 Pack linkage

`docs/ops/ACCEPTANCE_PACK_TEMPLATE.md` の Required Fields に `7a. Prompt-ID Cost Review` を置く。

- 7. `Gemini / Cost Impact` = ticket-level summary
- 7a. `Prompt-ID Cost Review` = per-prompt decomposition

prompt-id を触る ticket は 7 と 7a の両方が必要。7 だけ、または design doc だけでの代替は不可。

## 22. User Intervention Minimization / Weak HOLD Prevention(永続)

日々の役割・報告・HOLD・小さくGO判定の正本は `docs/ops/CURRENT_OPERATION_CONTRACT.md`。
本 section は詳細POLICYとの接続だけを残す。

- userはクライアント / 発注者であり、現場上司ではない。
- userへ戻すのは `USER_DECISION_REQUIRED` / P0-P1異常 / rollback / 不可逆変更 / 重要state到達 / 受け入れ判断のみ。
- HOLDは `ROLLBACK_BLOCKED` / `VERIFY_BLOCKED` / `USER_AUTH_REQUIRED` の1つに分類し、`HOLD_REASON` / `UNBLOCK_CONDITION` / `NEXT_OWNER` / `EXPIRY` / `USER_DECISION_REQUIRED` を書く。
- read-only / doc-only / preview-only / deployなし / flag-env-Scheduler-SEO-source変更なし / Gemini-mail増なし / WP本文変更なし / publish状態変更なし / rollback明確 / 衝突なしなら小さくGO候補。
- ChatGPTは外部コンサルであり、通常進行判断はClaudeが持つ。
- 282-COSTは、デグレ試験・rollback target・restore手順・stop condition・post-deploy verify・1 ticket単独実行が揃う時は小さくGO可能な前例。
