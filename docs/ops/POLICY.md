...
## 3. Claude 自律 GO 8 categories(USER_GO_REQUIRED=false で進める)
...
deploy / impl 着手 / 9 categories に到達した時点で **Acceptance Pack** を user に提示して `USER_GO_REQUIRED=true` 化。

### 即停止条件(自律 GO 中)

以下のいずれかを満たす場合、自律 GO を即停止:
- src / tests / config / Dockerfile / cloudbuild yaml / requirements.txt 編集が必要
- deploy / env / flag / Scheduler / secret 変更が必要
- Gemini call 増加が必要
- mail 通知条件変更が必要
- USER_GO_REQUIRED 領域に触れる
- P0 incident 検出 → user 即報告(自律対処しない)
...
## 14. P0/P1 incident 時の自律 hotfix 範囲(永続)

**mail storm / publish stuck / silent skip 急増 / Gemini call 急増 等の即時止血が必要な incident** に限り、以下の条件を **全部 AND で満たす場合**、user 明示 GO 受領後に **Claude 直接 gcloud 実行を許可**(Codex 便経由不要)。

### 必須条件(全部 AND、1 つでも違反したら Codex 便経由 + Acceptance Pack)

1. **scope narrow**: 1 service or 1 job の env / scheduler 操作のみ(複数同時禁止)
2. **code 変更なし**: src / tests / config / Dockerfile / cloudbuild yaml / requirements.txt 編集 0
3. **1 コマンド可逆**: `gcloud run jobs update --remove-env-vars=...` or `--update-env-vars=...` で 30 秒以内に元の状態に戻せる
4. **不変方針 維持**: §12 全項目(Team Shiny From / SEO / X / Scheduler 頻度 / live_update / 既存 prompt text)維持
5. **289 通知 / Gemini call 不変**: 観測導線を壊さない、Gemini call 増加させない
6. **publish-notice 全停止しない**: mail 全停止は別 ticket(`USER_GO_REASON=MAIL_ROUTING_MAJOR`)
7. **user 明示 GO 受領済**: 「P1 hotfix として env X を Y に変更してよい」等の明示文面が直前 user message に存在

### 実行後の必須記録(全部)

1. session log に 1 行追記: `HH:MM JST | hotfix | <ticket_id> | <env_action> | <next>`
2. OPS_BOARD.yaml `active:` に hotfix ticket を 1 件作成(retroactive OK、24h 内に追加)
3. 次 trigger 結果を read-only verify(sent / errors / Team Shiny From / 289 通知)
4. 完了報告 5 項目: env action / 次 trigger 結果 / errors / rollback 要否 / 影響範囲
...
### 通常の自律 GO 8 categories との関係

- §3「Claude 自律 GO 8 categories」(READ_ONLY 等)は env/flag 変更を **即停止条件** として規定
- 本 §14 は **user 明示 GO 受領後の限定例外**として、READ_ONLY+EVIDENCE_ONLY を超える narrow gcloud operation を許可
- 例外発動時 1 行返答必須: 「P1 hotfix として ... を実施します」を実行前に明示してから gcloud 実行

### 例(参考、2026-05-01 P1 mail storm hotfix)

```
[5/1 09:00 JST] user 明示「P0/P1 mail storm hotfixとして PUBLISH_NOTICE_REVIEW_WINDOW_HOURS=168 の env 追加を許可」
 → Claude 直接 `gcloud run jobs update publish-notice --update-env-vars=PUBLISH_NOTICE_REVIEW_WINDOW_HOURS=168` 実行 OK

[5/1 09:33 JST] user 明示「env 削除して default 24h に戻す GO」(storm trigger 仮説確認のため)
 → Claude 直接 `gcloud run jobs update publish-notice --remove-env-vars=PUBLISH_NOTICE_REVIEW_WINDOW_HOURS` 実行 OK

両方とも:scope narrow ✓ / code 変更 0 ✓ / 1 コマンド可逆 ✓ / 不変方針 維持 ✓ / Gemini 不変 ✓ / mail 全停止せず ✓ / user 明示 GO ✓
```
