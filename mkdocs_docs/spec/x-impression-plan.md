...
### Claude Code 案の弱点と Codex 補強

Claude Code 案は既存機能をよく整理しているが、 **インプを上げるための人間側の反応設計** が薄い。
以下は削除対象ではなく、 既存 Phase に追加して補強する観点。

| 弱点 | なぜ問題か | Codex 補強 |
| --- | --- | --- |
| 画像 / scheduler / API など仕組みの話が先に来る | X では仕組みより、 1 行目で止まるか・返信したくなるかが先 | 先に `post_text` の hook / reply candidate / 質問型を固める |
| 期待効果が「2-3 倍」「9 倍」など大きいが測定前提が弱い | 実測なしで施策を増やすと、 何が効いたか分からない | API なしでも user が mail から `伸びた/普通/弱い` を記録する |
| 投稿を増やす発想に寄りやすい | 薄い投稿を増やすと bot 感が出て逆効果 | 1 日の上限を維持し、 報知リプ / 自己補足 reply で表示場所を増やす |
| ハッシュタグ / 公式メンションを効果として見すぎ | 巨人ファン界隈では過剰タグ・過剰メンションが宣伝臭く見える | タグは最小限、 公式・報知には reply 候補で自然に入る |
| 自動投稿・server-side投稿が前に出やすい | X の live posting は失敗時のブランド毀損が大きい | 初期は candidate mail と手動投稿。 自動化は勝ち型確認後 |
| リプ戦略が弱い | インプは自分の投稿だけでなく、 反応の濃い投稿下に入ることで増える | `@hochi_giants` へのヨシラバー風リプ候補を新設 |
| negative example が少ない | 何を禁止するかが弱いと、 LLM が媒体転載・ポエムに戻る | NG例を test / prompt に入れる: 媒体名、掲示板suffix、感嘆だけ、長文返信 |
| 候補 mail の採用判断が user 任せすぎる | どれを投稿すべきか迷うと運用が止まる | 候補に `型` / `狙い` / `risk` / `推奨タイミング` を出す |

#### 追加すべき mail 表示

x-post-mail の各 candidate に、 投稿本文だけでなく次の metadata を出す。

| 表示項目 | 例 | 目的 |
| --- | --- | --- |
| `型` | `報知リプ` / `初速reply` / `データ補足` / `試合中本音` | user が用途で選べる |
| `狙い` | `報知投稿下の会話に入る` / `自投稿の初速補強` | なぜ出すか分かる |
| `推奨タイミング` | `報知投稿後30分以内` / `自投稿後10分以内` | インプが出やすい時間に合わせる |
| `risk` | `辛口なし` / `媒体名なし` / `自動返信禁止` | 炎上・spam を避ける |

これにより、 Claude Code 案の「仕組み中心」を、 Codex 側で **実際に投稿・返信を選びやすい運用中心** に補強する。
...
リプ候補を増やしても **追加費用は上げない**。

- 新規 Cloud Scheduler は作らない
- 新規 Cloud Run Job は作らない
- X API の自動返信は使わない
- Basic tier へ上げない
- 既存 RSSHub source (`@hochi_giants`) と既存 `x-post-mail-lane` の mail 枠に相乗りする
- 増えるのは mail 内の candidate 種別だけ。 user が手動で返信する

費用が上がる案は Phase 1 から外す。 自動返信 / X API read / インプ実測 API は、 手動リプの勝ち型が見えてから別判断。
...
費用を増やさない前提で、 まずは **1 日 3-5 リプ候補** を出す。

| 時間帯 | 候補数 | 狙い |
| --- | ---: | --- |
| 12:00 | 1 | 昼の報知記事・前日談話への落ち着いた補足 |
| 17:30 | 1-2 | スタメン / 試合前コメントへの反応 |
| 21:00-22:30 | 1-2 | 試合中・試合後の熱量が高い投稿への反応 |

上限は最初 5 件。 spam 感を避けるため、 同じ報知投稿に複数 reply は出さない。
手動投稿で「自然に返信できた」ものだけ採用し、 伸びた型を次週の few-shot に反映する。
