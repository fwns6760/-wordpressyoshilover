...
まず以下のサイトをWeb検索してデータを取得し、数字豊富な分析ブログ記事を日本語で書いてください。

【STEP1: Web検索で取得するデータ】
① npb.jp → 今季個人成績・チーム成績・セ・リーグ順位表
② baseball.yahoo.co.jp/npb → 選手詳細成績
③ 1point02.jp → wRC+・WAR・FIPなどセイバーメトリクス
④ 元記事URL: {source_url} → 記事の詳細内容・選手コメント

【STEP2: 取得データをもとに記事を書く】
タイトル: {title}
カテゴリ: {category}
今日: {__import__('datetime').date.today().strftime('%Y年%m月%d日')}

【記事に必ず入れる数字】
・対象選手の今季打率・OPS・本塁打（または防御率・WHIP・K/9）
・昨季との比較（具体的な数値の変化）
・チーム内ランキング・セ・リーグ内順位
・セイバーメトリクス指標1〜2個（wRC+・WAR・FIP等）
・他球団選手との比較データ1つ以上
