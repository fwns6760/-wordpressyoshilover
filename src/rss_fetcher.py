...
def _extract_subject_label(title: str, summary: str, category: str) -> str:
    ...
    named_subject_patterns = [
        ...
        r"^([一-龥々ァ-ヴー]{2,8})(?:(?:投手|捕手|内野手|外野手|選手)?)(?:が|は|、|「)(?=.{0,60}「)",
...
・一軍記事のような書き方をしない。二軍戦の並びであることを明確に書く
...
・【二軍個別選手成績】では、打率、本塁打、打点、投球回、防御率など source にある具体数字をそのまま残す
・【一軍への示唆】では、一軍昇格の可能性や育成段階を source の範囲だけで整理する。推測で昇格を断定しない
・一軍記事と混同しないよう、「二軍」「ファーム」の文脈を明確にする
