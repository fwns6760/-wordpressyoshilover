...
# 後半3つはエンゲ特化型 (2026-07-10 user「エンゲージ>インプ にトレンドポストが
# できるように工夫できる?」): 返信したくなる締め = 立場の断定 / 二択 / 検証可能な
# 予想。予想・断定も名前gate・接地gateの範囲内 (事実源にある人物・試合のみ)。
_TREND_ARRANGEMENTS = (
    ...
    "冒頭は事実の意外な一点を突く1行 → 根拠 → あえて強めの自分の断定 → 反対意見の"
    "余地を一言だけ残して締め (賛否が割れる形にする。直球の「どう思う?」は禁止)",
    "冒頭は論点の二択を提示する1行 (例: 続投か交代か) → 事実 → 両論に短く触れる → "
    "自分はどちら派か理由付きで断定して締め (読者が反対側に立ちたくなる形)",
    "冒頭はフック1行 → 事実 → 次の試合・次の打席の見どころ予想を1つだけ断定 → "
    "結果が出たら答え合わせしたくなる形で締め (予想は見出しと記事にある人物・試合の範囲のみ)",
)
            ...
            post_text = (
                build_quote_rt_comment(
                    ...
                    extra_voice_note=(
                        ...
                        "経歴・移籍・所属 (メジャー行き等)・数字・結果は絶対に"
                        "作らない。記事に登場しない人名 (他の選手・監督・コーチ・"
                        "OB・レジェンド) は一人も出さない。見出しに日付 (「11日」等) や「あす」が"
                        "ある時は、それがいつの話か本文に必ず明示する (今夜の"
                        "試合の話と誤読させない)。締めは読者が返信したくなる形"
                        "(自分の立場の明示、または賛否が割れる断定) にする。"
                        "ただし直球の「どう思う?」「皆さんは?」は禁止。"
                        f"今回の構成: {arrangement}。"
