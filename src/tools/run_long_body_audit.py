...
def scan_wp_drafts(*args, **kwargs):
    from src.long_body_compression_audit import scan_wp_drafts as impl

    return impl(*args, **kwargs)


def write_report(*args, **kwargs):
    from src.long_body_compression_audit import write_report as impl
...
def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="python3 -m src.tools.run_long_body_audit",
        description="Read-only audit for long-body draft compression or exclusion policy.",
    )
    parser.add_argument("--window-hours", type=int, default=72, help="Audit drafts modified within this many hours.")
