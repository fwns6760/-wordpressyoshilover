...
def _request_from_payload(payload: dict[str, Any]) -> PublishNoticeRequest:
    return PublishNoticeRequest(
        ...
        # 379-OPS (GH #53): publish_button_url も dry-run field 化
        publish_button_url=None if payload.get("publish_button_url") is None else str(payload.get("publish_button_url")),
    )
