...
- **【2026-06-04 B、 厳守】 数字を必ず 1 個入れる** (連勝数 / イニング / 失点 / 本数 / 打点 等、 DB fact line か記事内の数字のみ)。 その数字を「なぜ効くか・どこが効いたか・課題はどこか」 ファン目線で一言**読み解く**。 数字を並べるだけ・褒めるだけは禁止
- **【2026-06-04 B、 厳守】 当たり障りない優等生締め (「〜したいね」「期待して待ってる(よ)」「頼もしいよ」「楽しみだね」「成長してるね」 系) で終わらない**。 言い切り (〜だわ / 〜なんよな) / 見立て (〜になる気がする) / 本音 (次は打線、 頼むわ) / 問いかけ (〜どう見てる?) で締める
- 巨人以外の球団選手の話題は除外
...
def _build_system_prompt(now_jst_hour: int, today_jst: str, *, persona: str = "fuuga") -> str:
    ...
    if 5 <= now_jst_hour < 11:
        hint = (
            "【朝 5-11 時 = 試合前の立ち上げ。 熱量は落とさない (2026-06-02 user: 朝の静かに 撤廃)】\n"
            "- content: 前夜試合の余韻 / 連勝中の流れ / 今日試合への期待 / 先発予想\n"
            "- 熱量: 朝でも普通に熱く書く。 「静かに」「softer」 にしない\n"
            "- voice (例の語彙・リズム) は維持"
        )
...
# 門番 (品質ゲート): ポエム / スカスカ / 感嘆 spam だけを弾く。
# 2026-06-04 user 方針「辛口にしすぎない・温かさは OK」: 06-01 の良かった版は
# 「信じてる / 応援してる / 期待してる / してほしい」 等の温かい締めを持っていた。
# これらを弾く hopium 門番は撤去 (warmth を削って辛口すぎ=冷たくしていた)。
# 残すのは ① 中身ゼロのぬるい縮小締め「〜してほしいね/な」「頼もしいよな」 のみ、
# ② 抽象ポエム、 ③ スカスカ (短すぎ)、 ④ 感嘆符 spam。
_VOICE_YUTOUSEI_ENDING = _re.compile(
    # 当たり障りない優等生締め (褒めるだけ / 受け身の願望) を弾く。
    # 2026-06-04 B: 観測事故 (候補2「待ちたいね」/ 候補3「期待して待ってるよ」/「頼もしいよ」) を
    # 受けて拡張。 言い切り (〜だわ / 〜なんよな) / 見立て / 問いかけ / 本音 締めは引き続き許可。
    r"("
    r"してほしい(ね|な)(です)?|"        # ぬるい縮小形 (言い切り「してほしい」は許可)
    r"待ちたい(ね|な|よ)?|"             # 受け身の願望 (候補2)
    r"待ってる(ね|よ|な)?|"             # 「期待して待ってるよ」 (候補3)
    r"期待して(る|いる|ます|たい)(ね)?|"  # 受け身の優等生楽観
    r"頼もしい(よ|ね|な|限り|です)?|"     # 褒めるだけ (候補3)
    r"成長してる(ね|な)|"               # 褒めるだけ締め
    r"楽しみ(だ)?(ね|です(ね)?)"        # ぬるい (「楽しみすぎる」「早よ見たい」 は許可)
    r")"
    r"[。!！？\?…\s]*$"
)
# 2026-06-04 B: 数字 (半角/全角) を 1 個も含まない post は「褒めるだけ・中身なし」 として弾く。
# DB/記事内の連勝数 / イニング / 失点 / 本数 等を 1 個入れ、 その読みを書かせるため。
_VOICE_HAS_NUMBER = _re.compile(r"[0-9０-９]")
...
def _voice_quality_ok(text: str, *, live: bool = False) -> bool:
    ...
    if _VOICE_POEM_MARKERS.search(t):
        return False
    # 2026-06-04 B: 全時間帯で「数字 1 個 + その読み + 本音」 を必須化。 数字ゼロの post は
    # 褒めるだけ・中身なし として弾く (live でも適用、 缶詰短文も数字 1 個は要る)。
    if not _VOICE_HAS_NUMBER.search(t):
        return False
    # 薄い post (スカスカ・フィラー) を全時間帯で弾く。 live は短文 OK なので floor を下げる。
    if len(t) < (30 if live else 50):
        return False
    if not live:
        # 2026-06-04 口調を06-01へ復元: 06-01は「完勝！ 7連勝！！ ナイス！」式の感嘆連投が正。
        # 門番は感嘆を弾かず、 真の spam (6個以上) だけ落とす。
...
def build_quote_rt_comment(
    ...
    **フーガ (長文分析・試合後振り返り) + 缶詰 (試合中LIVE・連呼) 合成** をそのまま使う。
    ...
    if is_live:
        len_rule = (
            "80〜140字、 2〜3文。 試合中の熱量は出しつつ、 必ず "
            "①数字/事実を1個 (DB fact か元投稿内のイニング/連勝数/失点/本数 等) → "
            "②その数字をファン目線で一言読み解く (なぜ効く/どこが効いた) → ③本音で短く着地。 "
            "改行を1〜2個入れて読みやすく。 褒めるだけ・当たり障りない優等生締め "
            "(『〜したいね』『期待して待ってる』『頼もしいよ』『楽しみだね』 系) で終わらない。 "
            "元ネタに無い数字・事実は足さない。 "
            + _ending_style_hint(who)
        )
    else:
        len_rule = (
            "100〜170字、 2〜3文。 ★必ず適度に改行を入れ、 2〜3行の塊に分けて読みやすくする "
