...
- **長さ目安: 考察モードは 200〜320 文字 / 4〜6行** (Premium 長文、2026-07-07 user「結構長めに書いて」)。 長くする目的は記事の具体を拾うためであり、
...
def build_quote_rt_comment(
    ...
    **フーガ (長文分析・試合後振り返り) + 缶詰 (試合中LIVE・連呼) 合成** をそのまま使う。
    ...
    if is_empathy and (db_fact or "").strip():
        ...
        len_rule = (
            "動画SNS用。 120〜220字、 2〜3文。 元投稿の本文・映像に実際にある内容だけを核に反応する。 "
            "プレー動画なら場面を1つ具体名で書く "
        ...
        len_rule = (
            "220〜320字、 4〜6文。 ★必ず適度に改行を入れ、 2〜3行の塊に分けて読みやすくする "
    ...
    for attempt in range(_total_attempts):
        ...
        if attempt == 0:
            ...
            retry_note = (
                "※前回は抽象的/元投稿と無関係で却下された。 元投稿に実際にある内容だけに反応する。 "
                "プレー動画なら打球音・一歩目・送球・表情・ベンチ反応など目で見える要素から書く。 "
                "元投稿に無い場面を創作するな。 『見ておきたい』『楽しみ』『好投』だけで終わるな。\n"
            )
        else:
            retry_note = (
