...
- **争点+立場型を最優先 (2026-07-16 実測で最も反応が付いた型)**: source にポジション争い・
  レギュラー争い・打順論・先発/中継ぎ論の題材がある時は、 講評型より先にこの型を選ぶ。
  形 = 競合する2人を実名で並べる → 争いがチームに何をもたらすかを1つ → どちら派か自分の
  立場を置く (例Gの形)。 立場は断定しすぎず、 読者が反対側に立てる余白を残す
...
例G (争点 + 立場、 レギュラー争い型。 実測で最も反応が付いた型):
浦田がこれだけ結果を出すと、実績のある吉川尚輝もうかうかしてられないわ。
この二塁手争いが激しくなるほど打線も守備も締まるし、巨人は強くなる。
セカンドどっちで見たいか意見割れそうだけど、今は浦田が勝ってると思う。
    ...
    **フーガ (長文分析・試合後振り返り) + 缶詰 (試合中LIVE・連呼) 合成** をそのまま使う。
    ...
    if is_empathy and (db_fact or "").strip():
        ...
        len_rule = (
            ...
            # 2026-07-18 user「メジャーの情報間違い多い」: 実投稿で
            # (a) popped out to third を同一ポスト内で『三邪飛』と『サードフライ』の
            #     二通りに訳す自己矛盾、 (b) lineups are set を『選出おめでとう』と
            #     誤訳する時系列ズレが確認されたため、 訳語ルールを明示する。
            "★野球用語の訳: 打球の種類・守備位置の訳語に確信が持てない時は、 具体語に"
            "訳さず『凡退』『打ち取られた』等に抽象化する。 同じプレーを1ポスト内で"
            "二通りに訳さない。 英語の lineup は当日のスタメン (打順) の意味で、 "
            "オールスターやロースターの『選出』と訳さない。 "
        ...
        # 2026-07-14 user「選手がでてないのに (映像を見たかのような) コメントを書く。
        # コメントだけでもよいから内容をまちがえないで」: あなたに映像は見えていない
        # ことを明示し、場面描写は本文がプレーを伝えている時だけに限定する。
        # 記者・媒体のコメントが主の投稿は、コメント内容への反応だけで成立させてよい。
        len_rule = (
            "動画SNS用。 70〜120字、 1〜2文。 "
            "★あなたに映像の中身は見えていない。 事実として使ってよいのは元投稿の本文テキストだけ。 "
            "映像に映っている人物・動作・表情・音を、 本文の裏付けなしに見たかのように書かない "
            "(選手が映っていない動画も多い)。 "
            "本文がプレー (本塁打・好投・盗塁等) を伝えている時だけ、 そのプレーの場面価値を1つ書いてよい "
            "(何号 / 何回の場面 / 試合状況など本文にある要素で具体化する)。 "
            "記者・媒体・ファンのコメントや見解が主の投稿は、 そのコメント内容そのものに反応するだけで成立させてよい "
            "(無理に場面描写を作らない)。 "
            ...
            # 2026-07-18: SS型と同じ訳語ルール (誤訳→事実誤認の指摘が続いたため)。
            "英語の本文を訳して使う時、 打球の種類・守備位置の訳語に確信が持てなければ"
            "『凡退』等に抽象化し、 lineup を『選出』と訳さない。 "
        ...
        # 2026-07-16 fav実測 (7/13週次×全文join): fav上位10本は全て60〜123字、
        # 180字超は急落。既定を 120〜180字 (persona 正本の「考察モードは
        # 120-180 文字」と同値) へ短縮。
        len_rule = (
            "120〜180字、 2〜4文。 ★必ず適度に改行を入れ、 2〜3行の塊に分けて読みやすくする "
    ...
    for attempt in range(_total_attempts):
        ...
        # 2026-07-16 追加: 考察系 (非 live / 非リプ) の 50 字未満スカスカも致命的NG 扱い。
        # relaxed 許容で「李健熙は…あいさつを終えたか。」型の中身ゼロ 1 文が実投稿に漏れた実事故対応。
        is_fatal_thin = (not is_live) and (not is_reply) and len(text) < 50
        if (
            is_final_attempt
            and text
            and not unverified
            and not is_fatal_thin
