"""Tavily MCP (stdio 同梱) + Gemma 4 31B (Gemini API free tier) で
...
- 生成は Gemma 4 31B を Gemini API free tier 経由。 paid 課金は発生
  させない (user 0 ドル制約)。
...
# Gemini API 経由 Gemma 4 31B の model id.
# (ai.google.dev pricing page では "Gemma 4" 表記のみ、実 API では
#  size suffix 付き id 指定が必要。 SDK 実行時に実 id を smoke で確認。)
...
async def generate_post_drafts(
    *,
    ...
    パラメータ:
        ...
        model: Gemini API model id。 既定は Gemma 4 31B-it。
