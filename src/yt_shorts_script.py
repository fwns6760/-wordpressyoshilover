...
FAN_COMMENT_POOL: tuple[str, ...] = (
    ...
    # 2026-07-06 拡充 (user「良くなるところは直して」): 毎日見る人の
    # 「またこの言い回しか」を減らす。数字・事実なしの player 非依存文のみ。
    "こういう仕事ができる選手、ベンチは手放しません。",
    "数字にすると、働きの大きさがよく分かります。",
    "現地で見てたファンは、きっとうなずいてます。",
    "シーズン終盤、この積み重ねが効いてきます。",
    "派手なニュースの裏で、こういう数字が勝ちを作ってます。",
    "スタメン表を見る楽しみが、また増えました。",
    "こういう選手がいるチームは強い、って昔から言いますよね。",
    "ファン歴が長いほど、この数字の重みが分かるはず。",
    "気づいてる人は気づいてる、そんな活躍です。",
    "この調子なら、首脳陣の起用も変わってきそうです。",
    "数字は嘘をつかない、ってこういう時の言葉です。",
    "テレビ中継だけだと見逃しがちなポイントです。",
    "巨人の層の厚さは、こういうところに出ます。",
    "試合前に、ちょっと自慢したくなる数字です。",
    "こういうデータを追いかけるのが、ファンの醍醐味です。",
    "ベンチの信頼が数字ににじんでます。",
)
...
def build_script(topic: ShortsTopic) -> ShortsScript:
    ...
    # 結論ファースト (user「良くなるところは直して」): Shorts は冒頭1〜2秒で
    # スワイプ判定されるため「今日の注目は、」の司会前置きを廃止し、
    # 結論 (選手名+数字) から言い切る。記録日は seg2 (数字カード) へ退避。
    hook_seg = f"{speech_hook}"
    if not hook_seg.endswith("。"):
        hook_seg = f"{hook_seg}。"
    number_seg = f"{date_line}この数字、見逃せません。" if date_line else "この数字、見逃せません。"
    raw_segments = (
        hook_seg,
        number_seg,
        note_line,
