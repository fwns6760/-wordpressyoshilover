...
def test_related_posts_prioritize_player_match_then_same_subtype_then_same_category():
    player_posts = [
    ...
    assert [post["id"] for post in related] == [101, 201]


def test_related_posts_cut_off_older_than_30_days():
    player_posts = [
    ...
    assert [post["id"] for post in related] == [201]


def test_related_posts_exclude_current_article_and_duplicates():
    player_posts = [
        _post(
            101,
    ...
    assert [post["id"] for post in related] == [102, 201]


def test_related_posts_section_is_omitted_when_empty():
    assert rss_fetcher._build_related_posts_section([]) == ""


def test_build_news_block_inserts_related_posts_before_final_cta():
    with patch.object(
        rss_fetcher,
    ...
    assert "【関連記事】" in blocks
    assert 'class="yoshilover-related-posts"' in blocks
    assert blocks.index("【関連記事】") < blocks.index("今日のMVPは？")


def test_build_news_block_suppresses_empty_related_posts_section():
    ...
    assert "【関連記事】" not in blocks
    assert "yoshilover-related-posts" not in blocks
