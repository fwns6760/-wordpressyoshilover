...
def test_inline_notice_queue_jsonl_merge_deduplicates_sent_markers():
    existing = (
        '{"notice_kind":"per_post","post_id":1,"status":"sent","reason":"BATCH_SENT","sent_at":"2026-05-19T20:00:00+09:00"}\n'
    )
    local = existing + (
        '{"notice_kind":"per_post","post_id":2,"status":"sent","reason":"BATCH_SENT","sent_at":"2026-05-19T20:01:00+09:00"}\n'
    )

    merged = rss_fetcher._merge_inline_notice_queue_jsonl_texts(existing, local)

    assert merged.count('"post_id":1') == 1
    assert merged.count('"post_id":2') == 1
    assert merged.endswith("\n")


def test_inline_notice_remote_queue_upload_merges_with_generation(tmp_path, monkeypatch):
    class FakeBlob:
        ...
        def download_as_text(self, encoding: str = "utf-8") -> str:
            ...
            return (
                '{"notice_kind":"per_post","post_id":10,"status":"sent","sent_at":"2026-05-19T20:00:00+09:00"}\n'
            )
    ...
    class FakeBucket:
        ...
        def blob(self, name: str) -> FakeBlob:
            assert name == "publish_notice/queue.jsonl"
            return self._blob
    ...
    class FakeClient:
        ...
        def bucket(self, name: str) -> FakeBucket:
            assert name == "baseballsite-yoshilover-state"
            return FakeBucket(self._blob)

    fake_blob = FakeBlob()
    queue_path = tmp_path / "queue.jsonl"
    queue_path.write_text(
        '{"notice_kind":"per_post","post_id":11,"status":"sent","sent_at":"2026-05-19T20:01:00+09:00"}\n',
        encoding="utf-8",
    )
    monkeypatch.setattr(rss_fetcher, "_gcs_client", lambda: FakeClient(fake_blob))

    ok = rss_fetcher._upload_fetcher_inline_notice_remote_queue(
        queue_path,
        logging.getLogger("rss_fetcher"),
    )

    assert ok is True
    assert '"post_id":10' in fake_blob.uploaded
    assert '"post_id":11' in fake_blob.uploaded
    assert fake_blob.upload_kwargs["if_generation_match"] == 7
    assert fake_blob.upload_kwargs["content_type"] == "application/x-ndjson"
